...
ACPH_IMPLEMENT_RTTI_VIRTUAL(TaperedCylinderShapeTest)
{
	ACPH_ADD_BASE_CLASS(TaperedCylinderShapeTest, Test)
}
...
void TaperedCylinderShapeTest::Initialize()
{
	...
	mBodyInterface->CreateAndAddBody(BodyCreationSettings(big_taperedcylinder, RVec3(20, 10, 0), Quat::sRotation(Vec3::sAxisX(), 0.5f * ACPH_PI), EMotionType::Dynamic, Layers::MOVING), EActivation::Activate);
	...
	// Cone on side
	mBodyInterface->CreateAndAddBody(BodyCreationSettings(big_cone, RVec3(20, 10, 10), Quat::sRotation(Vec3::sAxisX(), 0.5f * ACPH_PI), EMotionType::Dynamic, Layers::MOVING), EActivation::Activate);
	...
	for (int i = 0; i < 10; ++i)
	{
		for (int j = 0; j < 2; ++j)
		{
			...
			if (i & 1)
			{
				...
				rotation = Quat::sRotation(Vec3::sAxisX(), 0.5f * ACPH_PI + (j & 1) * ACPH_PI);
			}
			else
			{
				position = RVec3(0, 2.0f + 3.0f * i, -20.0f - 4.0f + 8.0f * j);
				rotation = Quat::sRotation(Vec3::sAxisZ(), 0.5f * ACPH_PI + (j & 1) * ACPH_PI);
			}
