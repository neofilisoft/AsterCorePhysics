...
ACPH_IMPLEMENT_RTTI_VIRTUAL(MutableCompoundShapeTest)
{
	ACPH_ADD_BASE_CLASS(MutableCompoundShapeTest, Test)
}
...
void MutableCompoundShapeTest::Initialize()
{
	...
	sub_compound_settings.AddShape(Vec3(0, 1.5f, 0), Quat::sRotation(Vec3::sAxisZ(), 0.5f * ACPH_PI), new BoxShape(Vec3(1.5f, 0.25f, 0.2f)));
	sub_compound_settings.AddShape(Vec3(1.5f, 0, 0), Quat::sRotation(Vec3::sAxisZ(), 0.5f * ACPH_PI), new CylinderShape(1.5f, 0.2f));
	sub_compound_settings.AddShape(Vec3(0, 0, 1.5f), Quat::sRotation(Vec3::sAxisX(), 0.5f * ACPH_PI), new TaperedCapsuleShapeSettings(1.5f, 0.25f, 0.2f));
	...
	for (int i = 0; i < 10; ++i)
	{
		...
		compound_shape->AddShape(Vec3::sZero(), Quat::sRotation(Vec3::sAxisX(), -0.25f * ACPH_PI) * Quat::sRotation(Vec3::sAxisZ(), 0.25f * ACPH_PI), mSubCompound);
		compound_shape->AddShape(Vec3::sZero(), Quat::sRotation(Vec3::sAxisX(), 0.25f * ACPH_PI) * Quat::sRotation(Vec3::sAxisZ(), -0.75f * ACPH_PI), mSubCompound);
