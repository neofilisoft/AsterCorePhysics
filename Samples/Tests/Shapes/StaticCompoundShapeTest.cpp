...
ACPH_IMPLEMENT_RTTI_VIRTUAL(StaticCompoundShapeTest)
{
	ACPH_ADD_BASE_CLASS(StaticCompoundShapeTest, Test)
}
...
void StaticCompoundShapeTest::Initialize()
{
	...
	sub_compound->AddShape(Vec3(0, 1.5f, 0), Quat::sRotation(Vec3::sAxisZ(), 0.5f * ACPH_PI), new BoxShape(Vec3(1.5f, 0.25f, 0.2f)));
	sub_compound->AddShape(Vec3(1.5f, 0, 0), Quat::sRotation(Vec3::sAxisZ(), 0.5f * ACPH_PI), new CylinderShape(1.5f, 0.2f));
	sub_compound->AddShape(Vec3(0, 0, 1.5f), Quat::sRotation(Vec3::sAxisX(), 0.5f * ACPH_PI), new TaperedCapsuleShapeSettings(1.5f, 0.25f, 0.2f));

	Ref<StaticCompoundShapeSettings> compound_shape2 = new StaticCompoundShapeSettings;
	compound_shape2->AddShape(Vec3(0, 0, 0), Quat::sRotation(Vec3::sAxisX(), -0.25f * ACPH_PI) * Quat::sRotation(Vec3::sAxisZ(), 0.25f * ACPH_PI), sub_compound);
	compound_shape2->AddShape(Vec3(0, -0.1f, 0), Quat::sRotation(Vec3::sAxisX(), 0.25f * ACPH_PI) * Quat::sRotation(Vec3::sAxisZ(), -0.75f * ACPH_PI), sub_compound);
	...
	for (int y = -2; y <= 2; ++y)
		for (int x = -2; x <= 2; ++x)
			for (int z = -2; z <= 2; ++z)
				compound_shape3->AddShape(Vec3(0.5f * x, 0.5f * y, 0.5f * z), Quat::sRotation(Vec3::sAxisX(), -0.25f * ACPH_PI) * Quat::sRotation(Vec3::sAxisZ(), 0.25f * ACPH_PI), new BoxShape(Vec3::sReplicate(0.5f)));
	...
	for (int i = 0; i < 10; ++i)
		for (int j = 0; j < 3; ++j)
		{
			Quat rotation;
			if ((i & 1) == 0)
				rotation = Quat::sRotation(Vec3::sAxisX(), 0.5f * ACPH_PI);
			else
				rotation = Quat::sRotation(Vec3::sAxisZ(), 0.5f * ACPH_PI);
