...
ACPH_IMPLEMENT_RTTI_VIRTUAL(MeshShapeUserDataTest)
{
	ACPH_ADD_BASE_CLASS(MeshShapeUserDataTest, Test)
}
...
void MeshShapeUserDataTest::Initialize()
{
	...
	// Create body
	mBodyInterface->CreateAndAddBody(BodyCreationSettings(&compound_settings, RVec3::sZero(), Quat::sRotation(Vec3::sAxisX(), 0.25f * ACPH_PI), EMotionType::Static, Layers::NON_MOVING), EActivation::DontActivate);

	// 1 body with zero friction
	BodyCreationSettings bcs(new BoxShape(Vec3::sReplicate(2.0f)), RVec3(0, 55.0f, -50.0f), Quat::sRotation(Vec3::sAxisX(), 0.25f * ACPH_PI), EMotionType::Dynamic, Layers::MOVING);
	bcs.mFriction = 0.0f;
