...
ACPH_IMPLEMENT_RTTI_VIRTUAL(MeshShapeTest)
{
	ACPH_ADD_BASE_CLASS(MeshShapeTest, Test)
}
...
void MeshShapeTest::Initialize()
{
	...
	// Floor
	mBodyInterface->CreateAndAddBody(BodyCreationSettings(new MeshShapeSettings(triangles, std::move(materials)), RVec3::sZero(), Quat::sRotation(Vec3::sAxisX(), 0.25f * ACPH_PI), EMotionType::Static, Layers::NON_MOVING), EActivation::DontActivate);
	...
	Body &body = *mBodyInterface->CreateBody(BodyCreationSettings(box_shape, RVec3(0, 55.0f, -50.0f), Quat::sRotation(Vec3::sAxisX(), 0.25f * ACPH_PI), EMotionType::Dynamic, Layers::MOVING));
