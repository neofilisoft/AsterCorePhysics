...
ACPH_IMPLEMENT_RTTI_VIRTUAL(SoftBodyCosseratRodConstraintTest)
{
	ACPH_ADD_BASE_CLASS(SoftBodyCosseratRodConstraintTest, Test)
}
...
void SoftBodyCosseratRodConstraintTest::Initialize()
{
	...
	// Create a hanging helix
	{
		...
		for (int i = 0; i < cNumVertices; ++i)
		{
			...
			float alpha = cNumCycles * 2.0f * ACPH_PI * fraction;
	...
	// Create a weed like structure
	{
		...
		uniform_real_distribution<float> phase_distribution(0, 2.0f * ACPH_PI);
		...
		for (int strand = 0; strand < cNumStrands; ++strand)
		{
			...
			for (int i = 0; i < cNumVertices; ++i)
			{
				...
				float amplitude = 0.1f * Sin(phase1 + i * 2.0f * ACPH_PI / 8);
