...
ACPH_IMPLEMENT_RTTI_VIRTUAL(SoftBodySkinnedConstraintTest)
{
	ACPH_ADD_BASE_CLASS(SoftBodySkinnedConstraintTest, Test)
}
...
Array<Mat44> SoftBodySkinnedConstraintTest::GetWorldSpacePose(float inTime) const
{
	...
	for (int i = 1; i < cNumJoints; ++i)
	{
		...
		Mat44 rotation = Mat44::sRotationX(amplitude * Sin(0.25f * ACPH_PI * i + 2.0f * inTime));
