...
ACPH_IMPLEMENT_RTTI_VIRTUAL(ShapeFilterTest)
{
	ACPH_ADD_BASE_CLASS(ShapeFilterTest, Test)
}
...
void ShapeFilterTest::PostPhysicsUpdate(float inDeltaTime)
{
	...
	float shape_select = fmod(phase, 6.0f * ACPH_PI);
	const char *text;
	if (shape_select < 2.0f * ACPH_PI)
	{
		shape_filter.mUserDataOfShapeToIgnore = (uint64)ShapeIdentifier::Box;
		text = "Box";
	}
	else if (shape_select < 4.0f * ACPH_PI)
	{
	...
	ACPH_IF_DEBUG_RENDERER(mCastShape->Draw(mDebugRenderer, RMat44::sTranslation(RVec3(cast_point)), Vec3::sOne(), color, false, true);)
}
