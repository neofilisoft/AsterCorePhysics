...
ACPH_IMPLEMENT_RTTI_VIRTUAL(FunnelTest)
{
	ACPH_ADD_BASE_CLASS(FunnelTest, Test)
}
...
void FunnelTest::Initialize()
{
	...
	for (int i = 0; i < 4; ++i)
	{
		...
		mBodyInterface->CreateAndAddBody(BodyCreationSettings(box, RVec3(rotation * Vec3(25, 25, 0)), rotation * Quat::sRotation(Vec3::sAxisZ(), 0.25f * ACPH_PI), EMotionType::Static, Layers::NON_MOVING), EActivation::DontActivate);
	}
		...
		case 9:
			{
				...
				sub_compound->AddShape(Vec3(0, 0.75f, 0), Quat::sRotation(Vec3::sAxisZ(), 0.5f * ACPH_PI), new BoxShape(Vec3(0.75f, 0.25f, 0.2f)));
				sub_compound->AddShape(Vec3(0.75f, 0, 0), Quat::sRotation(Vec3::sAxisZ(), 0.5f * ACPH_PI), new CylinderShape(0.75f, 0.2f));
				sub_compound->AddShape(Vec3(0, 0, 0.75f), Quat::sRotation(Vec3::sAxisX(), 0.5f * ACPH_PI), new TaperedCapsuleShapeSettings(0.75f, 0.25f, 0.2f));
				StaticCompoundShapeSettings compound_shape_settings;
				compound_shape_settings.AddShape(Vec3(0, 0, 0), Quat::sRotation(Vec3::sAxisX(), -0.25f * ACPH_PI) * Quat::sRotation(Vec3::sAxisZ(), 0.25f * ACPH_PI), sub_compound);
				compound_shape_settings.AddShape(Vec3(0, -0.1f, 0), Quat::sRotation(Vec3::sAxisX(), 0.25f * ACPH_PI) * Quat::sRotation(Vec3::sAxisZ(), -0.75f * ACPH_PI), sub_compound);
				shape = compound_shape_settings.Create().Get();
				scale = scale.Swizzle<SWIZZLE_X, SWIZZLE_X, SWIZZLE_X>(); // Only uniform scale supported
				break;
			}
