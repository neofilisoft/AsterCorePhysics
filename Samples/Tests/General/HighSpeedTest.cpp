...
ACPH_IMPLEMENT_RTTI_VIRTUAL(HighSpeedTest)
{
	ACPH_ADD_BASE_CLASS(HighSpeedTest, Test)
}
...
void HighSpeedTest::CreateDominoBlocks(RVec3Arg inOffset, int inNumWalls, float inDensity, float inRadius)
{
	...
	box_settings.mRotation = Quat::sRotation(Vec3::sAxisY(), 0.5f * ACPH_PI);
	mBodyInterface->CreateAndAddBody(box_settings, EActivation::DontActivate);
}
...
void HighSpeedTest::CreateSimpleScene()
{
...

	{
		...
		enclosing_shape->AddShape(Vec3(0, 0, -1), Quat::sIdentity(), box_shape);
		enclosing_shape->AddShape(Vec3(1, 0, 0), Quat::sRotation(Vec3::sAxisY(), 0.5f * ACPH_PI), box_shape);
		enclosing_shape->AddShape(Vec3(-1, 0, 0), Quat::sRotation(Vec3::sAxisY(), 0.5f * ACPH_PI), box_shape);
...

	{
		...
		BodyCreationSettings body_settings(&box_settings, offset + Vec3(0, 1, 0), Quat::sRotation(Vec3::sAxisX(), 0.5f * ACPH_PI), EMotionType::Dynamic, Layers::MOVING);
...

	{
		...
		BoxShapeSettings box_settings(Vec3(0.05f, 0.8f, 0.03f), 0.015f);
		box_settings.SetEmbedded();
		BodyCreationSettings body_settings(&box_settings, offset + Vec3(0, 1, 0), Quat::sRotation(Vec3::sAxisX(), 0.25f * ACPH_PI), EMotionType::Dynamic, Layers::MOVING);
		body_settings.mMotionQuality = EMotionQuality::LinearCast;
		body_settings.mRestitution = 0.0f;
		body_settings.mFriction = 1.0f;
