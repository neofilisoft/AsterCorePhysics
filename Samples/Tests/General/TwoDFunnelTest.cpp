...
ACPH_IMPLEMENT_RTTI_VIRTUAL(TwoDFunnelTest)
{
	ACPH_ADD_BASE_CLASS(TwoDFunnelTest, Test)
}
...
void TwoDFunnelTest::Initialize()
{
	...
	// 2D funnel
	mBodyInterface->CreateAndAddBody(BodyCreationSettings(wall, RVec3(-12, 8, -5), Quat::sRotation(Vec3::sAxisZ(), 0.2f * ACPH_PI), EMotionType::Static, Layers::NON_MOVING), EActivation::DontActivate);
	mBodyInterface->CreateAndAddBody(BodyCreationSettings(wall, RVec3(12, 8, -5), Quat::sRotation(Vec3::sAxisZ(), -0.2f * ACPH_PI), EMotionType::Static, Layers::NON_MOVING), EActivation::DontActivate);
