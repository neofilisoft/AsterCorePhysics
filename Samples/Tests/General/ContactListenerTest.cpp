...
#include <AsterCore/Physics/Collision/EstimateCollisionResponse.h>
#include <AsterCore/Physics/Body/BodyCreationSettings.h>
#include <Layers.h>
#include <Renderer/DebugRendererImp.h>

ACPH_IMPLEMENT_RTTI_VIRTUAL(ContactListenerTest)
{
	ACPH_ADD_BASE_CLASS(ContactListenerTest, Test)
}
...
void ContactListenerTest::Initialize()
{
	...
	// Dynamic body 2
	Body &body2 = *mBodyInterface->CreateBody(BodyCreationSettings(box_shape, RVec3(5, 10, 0), Quat::sRotation(Vec3::sAxisX(), 0.25f * ACPH_PI), EMotionType::Dynamic, Layers::MOVING));
	body2.SetAllowSleeping(false);
	mBodyInterface->AddBody(body2.GetID(), EActivation::Activate);

	// Dynamic body 3
	Body &body3 = *mBodyInterface->CreateBody(BodyCreationSettings(new SphereShape(2.0f), RVec3(10, 10, 0), Quat::sRotation(Vec3::sAxisX(), 0.25f * ACPH_PI), EMotionType::Dynamic, Layers::MOVING));
	...
	compound_shape->AddShape(Vec3(0, 5, 0), Quat::sIdentity(), new SphereShape(2));
	Body &body4 = *mBodyInterface->CreateBody(BodyCreationSettings(compound_shape, RVec3(15, 10, 0), Quat::sRotation(Vec3::sAxisX(), 0.25f * ACPH_PI), EMotionType::Dynamic, Layers::MOVING));
...
void ContactListenerTest::OnContactAdded(const Body &inBody1, const Body &inBody2, const ContactManifold &inManifold, ContactSettings &ioSettings)
{
	...
	{
		ACPH_ASSERT(ioSettings.mCombinedRestitution == 0.0f);
