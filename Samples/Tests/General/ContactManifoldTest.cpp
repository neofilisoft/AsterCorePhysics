...
ACPH_IMPLEMENT_RTTI_VIRTUAL(ContactManifoldTest)
{
	ACPH_ADD_BASE_CLASS(ContactManifoldTest, Test)
}
...
void ContactManifoldTest::Initialize()
{
	...
	for (int i = 0; i < 3; ++i)
		for (int j = 0; j < 2; ++j)
		{
			...
			mBodyInterface->CreateAndAddBody(BodyCreationSettings(j == 0? capsule : long_box, RVec3(-20.0f + i * 10.0f, 12, -5.0f + i * 5.0f - 20.0f + j * 40.0f), Quat::sRotation(Vec3::sAxisY(), 0.1f * ACPH_PI) * Quat::sRotation(Vec3::sAxisX(), 0.5f * ACPH_PI), EMotionType::Dynamic, Layers::MOVING), EActivation::Activate);
		}
}
