...
ACPH_IMPLEMENT_RTTI_VIRTUAL(ChangeMotionQualityTest)
{
	ACPH_ADD_BASE_CLASS(ChangeMotionQualityTest, Test)
}
...
void ChangeMotionQualityTest::Initialize()
{
	...
	enclosing_shape->AddShape(Vec3(0, 0, -5), Quat::sIdentity(), box_shape);
	enclosing_shape->AddShape(Vec3(5, 0, 0), Quat::sRotation(Vec3::sAxisY(), 0.5f * ACPH_PI), box_shape);
	enclosing_shape->AddShape(Vec3(-5, 0, 0), Quat::sRotation(Vec3::sAxisY(), 0.5f * ACPH_PI), box_shape);
