...
ACPH_IMPLEMENT_RTTI_VIRTUAL(ConveyorBeltTest)
{
	ACPH_ADD_BASE_CLASS(ConveyorBeltTest, Test)
}
...
void ConveyorBeltTest::Initialize()
{
	...
	for (int i = 0; i < 4; ++i)
	{
		...
		belt_settings.mRotation = Quat::sRotation(Vec3::sAxisY(), 0.5f * ACPH_PI * i) * Quat::sRotation(Vec3::sAxisX(), DegreesToRadians(1.0f));
	...
	// Create 2 cylinders
	BodyCreationSettings cylinder_settings(new CylinderShape(6.0f, 1.0f), RVec3(-25.0f, 1.0f, -20.0f), Quat::sRotation(Vec3::sAxisZ(), 0.5f * ACPH_PI), EMotionType::Dynamic, Layers::MOVING);
