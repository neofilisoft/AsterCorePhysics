...
ACPH_IMPLEMENT_RTTI_VIRTUAL(MultithreadedTest)
{
	ACPH_ADD_BASE_CLASS(MultithreadedTest, Test)
}
...
{
	ACPH_PROFILE_THREAD_START("BoxSpawner");
...
{
	ACPH_PROFILE_THREAD_START("RagdollSpawner");
	...
	// Load ragdoll
	Ref<RagdollSettings> ragdoll_settings = RagdollLoader::sLoad("Entity.tof", EMotionType::Dynamic);
	...
	ragdoll_pose.SetSkeleton(ragdoll_settings->GetSkeleton());
	{
		...
		AssetStream stream("Entity/dead_pose1.tof", std::ios::in);
...
{
	ACPH_PROFILE_THREAD_START("CasterMain");
