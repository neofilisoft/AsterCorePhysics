...
ACPH_IMPLEMENT_RTTI_VIRTUAL(FrictionPerTriangleTest)
{
	ACPH_ADD_BASE_CLASS(FrictionPerTriangleTest, Test)
}
...
void FrictionPerTriangleTest::Initialize()
{
	...
	// A ramp
	mBodyInterface->CreateAndAddBody(BodyCreationSettings(new MeshShapeSettings(triangles, std::move(materials)), RVec3::sZero(), Quat::sRotation(Vec3::sAxisX(), 0.2f * ACPH_PI), EMotionType::Static, Layers::NON_MOVING), EActivation::DontActivate);
	...
	mBodyInterface->CreateAndAddBody(BodyCreationSettings(box_shape, RVec3(0, 60.0f, -75.0f), Quat::sRotation(Vec3::sAxisX(), 0.2f * ACPH_PI), EMotionType::Dynamic, Layers::MOVING), EActivation::Activate);
}
