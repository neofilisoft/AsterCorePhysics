...
ACPH_IMPLEMENT_RTTI_VIRTUAL(SimShapeFilterTest)
{
	ACPH_ADD_BASE_CLASS(SimShapeFilterTest, Test)
}
...
void SimShapeFilterTest::Initialize()
{
	...
	// Platform
	mShapeFilter.mPlatformID = mBodyInterface->CreateAndAddBody(BodyCreationSettings(new BoxShape(Vec3(5.0f, 0.5f, 5.0f)), RVec3(0, 7.5f, 0), Quat::sRotation(Vec3::sAxisX(), 0.25f * ACPH_PI), EMotionType::Static, Layers::NON_MOVING), EActivation::DontActivate);
