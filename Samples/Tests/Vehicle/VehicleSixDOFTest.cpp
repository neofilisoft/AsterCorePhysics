...
ACPH_IMPLEMENT_RTTI_VIRTUAL(VehicleSixDOFTest)
{
	ACPH_ADD_BASE_CLASS(VehicleSixDOFTest, VehicleTest)
}
...
void VehicleSixDOFTest::Initialize()
{
	...
	for (int i = 0; i < (int)EWheel::Num; ++i)
	{
		...
		// Create body
		Body &wheel = *mBodyInterface->CreateBody(BodyCreationSettings(wheel_shape, wheel_pos2, Quat::sRotation(Vec3::sAxisZ(), 0.5f * ACPH_PI), EMotionType::Dynamic, Layers::MOVING));
...
{
	const float max_rotation_speed = 10.0f * ACPH_PI;
