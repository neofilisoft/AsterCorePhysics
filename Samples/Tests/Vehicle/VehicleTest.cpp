...
ACPH_IMPLEMENT_RTTI_VIRTUAL(VehicleTest)
{
	ACPH_ADD_BASE_CLASS(VehicleTest, Test)
}
...
void VehicleTest::Initialize()
{
	...
	else if (strcmp(sSceneName, "Step") == 0)
	{
		...
		Body &step = *mBodyInterface->CreateBody(BodyCreationSettings(new BoxShape(Vec3(5.0f, 0.5f * cStepHeight, 5.0f), 0.0f), RVec3(-2.0f, 0.5f * cStepHeight, 60.0f), Quat::sRotation(Vec3::sAxisY(), -0.3f * ACPH_PI), EMotionType::Static, Layers::NON_MOVING));
	...
	else if (strcmp(sSceneName, "Loop") == 0)
	{
		...
		{
			float angle = i * 2.0f * ACPH_PI / (cNumSegments - 1);
