...
ACPH_IMPLEMENT_RTTI_VIRTUAL(DynamicScaledShape)
{
	ACPH_ADD_BASE_CLASS(DynamicScaledShape, Test)
}
...
void DynamicScaledShape::PrePhysicsUpdate(const PreUpdateParams &inParams)
{
	...
	if (lock.Succeeded())
	{
		...
		ACPH_ASSERT(body.GetShape()->GetSubType() == EShapeSubType::Scaled);
		...
		ACPH_ASSERT(new_shape.IsValid()); // We're uniformly scaling a sphere, this should always succeed
