...
ACPH_IMPLEMENT_RTTI_VIRTUAL(ScaledOffsetCenterOfMassShapeTest)
{
	ACPH_ADD_BASE_CLASS(ScaledOffsetCenterOfMassShapeTest, Test)
}
...
void ScaledOffsetCenterOfMassShapeTest::Initialize()
{
	...
	Quat rotation = Quat::sRotation(Vec3::sAxisZ(), 0.4f * ACPH_PI);
	...
	ShapeRefC pre_scaled = OffsetCenterOfMassShapeSettings(Vec3(0, 0, 5.0f), new ScaledShape(new SphereShape(1.0f), ACPH::Vec3::sReplicate(2.0f))).Create().Get();
	...
	ShapeRefC post_scaled = new ScaledShape(OffsetCenterOfMassShapeSettings(Vec3(0, 0, 5.0f), new SphereShape(1.0f)).Create().Get(), ACPH::Vec3::sReplicate(2.0f));
