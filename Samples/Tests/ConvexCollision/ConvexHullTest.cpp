...
ACPH_IMPLEMENT_RTTI_VIRTUAL(ConvexHullTest)
{
	ACPH_ADD_BASE_CLASS(ConvexHullTest, Test)
}
...
void ConvexHullTest::Initialize()
{
	...
	{
		Points p;
		Mat44 rot = Mat44::sRotationZ(0.25f * ACPH_PI);
		for (float r = 0.0f; r < 2.0f; r += 0.1f)
			for (float phi = 0.0f; phi <= 2.0f * ACPH_PI; phi += 2.0f * ACPH_PI / 20.0f)
	...
	{
		Points p;
		for (float phi = 0.0f; phi <= 2.0f * ACPH_PI; phi += 2.0f * ACPH_PI / 40.0f)
		{
	...
	{
		Points p;
		for (float theta = 0.0f; theta <= ACPH_PI; theta += ACPH_PI / 20.0f)
			for (float phi = 0.0f; phi <= 2.0f * ACPH_PI; phi += 2.0f * ACPH_PI / 20.0f)
