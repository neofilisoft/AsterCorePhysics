...
ACPH_IMPLEMENT_RTTI_VIRTUAL(CharacterVirtualTest)
{
	ACPH_ADD_BASE_CLASS(CharacterVirtualTest, CharacterBaseTest)
}
...
void CharacterVirtualTest::PrePhysicsUpdate(const PreUpdateParams &inParams)
{
	...
	for (const CharacterVirtual::Contact &c : mCharacter->GetActiveContacts())
		if (c.mHadCollision)
		{
			ACPH_ASSERT(std::find(mActiveContacts.begin(), mActiveContacts.end(), c) != mActiveContacts.end());
			num_contacts++;
		}
	ACPH_ASSERT(sShapeType == EType::Compound? num_contacts >= mActiveContacts.size() : num_contacts == mActiveContacts.size());
#endif
