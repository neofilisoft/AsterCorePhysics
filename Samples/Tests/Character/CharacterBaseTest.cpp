...
ACPH_IMPLEMENT_RTTI_ABSTRACT(CharacterBaseTest)
{
	ACPH_ADD_BASE_CLASS(CharacterBaseTest, Test)
}
...
static const Quat cHorizontallyMovingOrientation = Quat::sRotation(Vec3::sAxisZ(), 0.5f * ACPH_PI);
static const RVec3 cConveyorBeltPosition(-10, 0.15f, 15);
static const RVec3 cRampPosition(15, 2.2f, 15);
static const Quat cRampOrientation = Quat::sRotation(Vec3::sAxisX(), -0.25f * ACPH_PI);
...
void CharacterBaseTest::Initialize()
{
	...
	else if (strcmp(sSceneName, "ObstacleCourse") == 0)
	{
		...
		for (int i = 0; i < 2; ++i)
		{
			Quat rotation = Quat::sRotation(Vec3::sAxisY(), ACPH_PI * i);
		...
		for (int i = 0; i < 3; ++i)
		{
			Quat rotation = Quat::sRotation(Vec3::sAxisY(), 2.0f / 3.0f * ACPH_PI * i);
		...
		for (int i = 0; i < 4; ++i)
		{
			Quat rotation = Quat::sRotation(Vec3::sAxisY(), 0.5f * ACPH_PI * i);
		...
		// Create a half cylinder with caps for testing contact point limit
		{
			...
			for (int pos = 0; pos < cPosSegments; ++pos)
				for (int angle = 0; angle < cAngleSegments; ++angle)
				{
					...
					float angle_rad = (-0.5f + float(angle) / cAngleSegments) * ACPH_PI;
...
void CharacterBaseTest::PrePhysicsUpdate(const PreUpdateParams &inParams)
{
	...
	if (!mRotatingBody.IsInvalid())
		mBodyInterface->MoveKinematic(mRotatingBody, cRotatingPosition, Quat::sRotation(Vec3::sAxisY(), ACPH_PI * Sin(mTime)), inParams.mDeltaTime);
	if (!mRotatingWallBody.IsInvalid())
		mBodyInterface->MoveKinematic(mRotatingWallBody, cRotatingWallPosition, Quat::sRotation(Vec3::sAxisY(), ACPH_PI * Sin(mTime)), inParams.mDeltaTime);
	if (!mRotatingAndTranslatingBody.IsInvalid())
		mBodyInterface->MoveKinematic(mRotatingAndTranslatingBody, cRotatingAndTranslatingPosition + 5.0f * Vec3(Sin(ACPH_PI * mTime), 0, Cos(ACPH_PI * mTime)), Quat::sRotation(Vec3::sAxisY(), ACPH_PI * Sin(mTime)), inParams.mDeltaTime);
