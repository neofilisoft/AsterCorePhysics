...
ACPH_IMPLEMENT_RTTI_VIRTUAL(SkeletonMapperTest)
{
	ACPH_ADD_BASE_CLASS(SkeletonMapperTest, Test)
}
...
void SkeletonMapperTest::Initialize()
{
	...
	// Load ragdoll
	mRagdollSettings = RagdollLoader::sLoad("Entity.tof", EMotionType::Dynamic);
	...
	{
		AssetStream stream("Entity/neutral.tof", std::ios::in);
	...
	{
		AssetStream stream("Entity/skeleton_hd.tof", std::ios::in);
	...
	{
		AssetStream stream("Entity/neutral_hd.tof", std::ios::in);
	...
	// Load test animation
	{
		AssetStream stream("Entity/jog_hd.tof", std::ios::in);
