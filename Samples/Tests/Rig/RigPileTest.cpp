...
ACPH_IMPLEMENT_RTTI_VIRTUAL(RigPileTest)
{
	ACPH_ADD_BASE_CLASS(RigPileTest, Test)
}
...
void RigPileTest::Initialize()
{
	...
	// Load ragdoll
	Ref<RagdollSettings> settings = RagdollLoader::sLoad("Entity.tof", EMotionType::Dynamic);
	...
	{
		AssetStream stream(StringFormat("Entity/dead_pose%d.tof", i + 1), std::ios::in);
	...
	uniform_real_distribution<float> angle(0.0f, ACPH_PI);
