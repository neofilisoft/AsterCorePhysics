...
ACPH_IMPLEMENT_RTTI_VIRTUAL(PathConstraintTest)
{
	ACPH_ADD_BASE_CLASS(PathConstraintTest, Test)
}
...
void PathConstraintTest::Initialize()
{
...

	{
		...
		for (float a = -0.1f * ACPH_PI; a < 4.0f * ACPH_PI; a += 0.1f * ACPH_PI)
...

	{
		...
		settings.mPathRotation = Quat::sRotation(Vec3::sAxisX(), -0.1f * ACPH_PI);
		settings.mRotationConstraintType = sOrientationType;
