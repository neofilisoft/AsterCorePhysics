...
ACPH_IMPLEMENT_RTTI_VIRTUAL(SwingTwistConstraintTest)
{
	ACPH_ADD_BASE_CLASS(SwingTwistConstraintTest, Test)
}
...
void SwingTwistConstraintTest::Initialize()
{
	...
	for (int i = 0; i < cChainLength; ++i)
	{
		...
		Body &segment = *mBodyInterface->CreateBody(BodyCreationSettings(new CapsuleShape(half_cylinder_height, 0.5f), position, Quat::sRotation(Vec3::sAxisX(), 0.25f * ACPH_PI * i) * rotation, i == 0? EMotionType::Static : EMotionType::Dynamic, i == 0? Layers::NON_MOVING : Layers::MOVING));
