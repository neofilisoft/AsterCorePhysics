...
ACPH_IMPLEMENT_RTTI_VIRTUAL(RackAndPinionConstraintTest)
{
	ACPH_ADD_BASE_CLASS(RackAndPinionConstraintTest, Test)
}
...
void RackAndPinionConstraintTest::Initialize()
{
	...
	constexpr int cRackNumTeeth = int(cRackLength * cGearNumTeeth / (2.0f * ACPH_PI * cGearRadius));
	...
	{
		Quat rotation = Quat::sRotation(Vec3::sAxisY(), 2.0f * ACPH_PI * i / cGearNumTeeth);
		gear_settings.AddShape(rotation * Vec3(cGearRadius, 0, 0), rotation, &tooth_settings);
	}

	RVec3 gear_initial_p(0, 2.0f, 0);
	Quat gear_initial_r = Quat::sRotation(Vec3::sAxisX(), 0.5f * ACPH_PI);
	...
	Quat slider_initial_r = Quat::sRotation(Vec3::sAxisZ(), 0.5f * ACPH_PI) * gear_initial_r;
