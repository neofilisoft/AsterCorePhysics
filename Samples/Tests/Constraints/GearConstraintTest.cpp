...
ACPH_IMPLEMENT_RTTI_VIRTUAL(GearConstraintTest)
{
	ACPH_ADD_BASE_CLASS(GearConstraintTest, Test)
}
...
void GearConstraintTest::Initialize()
{
	...
	{
		Quat rotation = Quat::sRotation(Vec3::sAxisY(), 2.0f * ACPH_PI * i / cGear1NumTeeth);
		gear1_settings.AddShape(rotation * Vec3(cGear1Radius, 0, 0), rotation, &tooth_settings);
	}

	RVec3 gear1_initial_p(0, 3.0f, 0);
	Quat gear1_initial_r = Quat::sRotation(Vec3::sAxisX(), 0.5f * ACPH_PI);
	...
	{
		Quat rotation = Quat::sRotation(Vec3::sAxisY(), 2.0f * ACPH_PI * (i + 0.5f) / cGear2NumTeeth);
