...
#include <AsterCore/Physics/Constraints/FixedConstraint.h>
#include <AsterCore/Physics/Body/BodyCreationSettings.h>
#include <Layers.h>

ACPH_IMPLEMENT_RTTI_VIRTUAL(FixedConstraintTest)
{
	ACPH_ADD_BASE_CLASS(FixedConstraintTest, Test)
}
...
void FixedConstraintTest::Initialize()
{
...

	{
		...
		Quat base_rotation = Quat::sRotation(Vec3::sAxisZ(), -0.5f * ACPH_PI);
		...
		for (int y = 0; y < 10; ++y)
		{
			...
			for (int i = 0; i < 4; ++i)
			{
				Quat rotation = Quat::sRotation(Vec3::sAxisY(), i * 0.5f * ACPH_PI);
			...
			for (int i = 0; i < 4; ++i)
			{
				...
				// Create cross beam
				Body *cross = mBodyInterface->CreateBody(BodyCreationSettings(beam, base_position + base_rotation * (center + rotation * Vec3(1.105f, 1.0f, 0.0f)), base_rotation * rotation * Quat::sRotation(Vec3::sAxisX(), 0.3f * ACPH_PI), EMotionType::Dynamic, Layers::MOVING));
				...
				for (int j = 0; j < 2; ++j)
				{
					...
					constraint.mNumPositionStepsOverride = ACPH_IF_NOT_DEBUG(64) ACPH_IF_DEBUG(8); // In debug mode use less steps to preserve framerate (at the cost of stability)
				...
				if (prev_pillars[i] != nullptr)
				{
					...
					constraint.mNumPositionStepsOverride = ACPH_IF_NOT_DEBUG(64) ACPH_IF_DEBUG(8);
