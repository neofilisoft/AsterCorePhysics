...
ACPH_IMPLEMENT_RTTI_VIRTUAL(ConeConstraintTest)
{
	ACPH_ADD_BASE_CLASS(ConeConstraintTest, Test)
}
...
void ConeConstraintTest::Initialize()
{
	...
	{
		Body *prev = nullptr;
		Quat rotation = Quat::sRotation(Vec3::sAxisZ(), 0.5f * ACPH_PI);
		RVec3 position(0, 20.0f, 10.0f * j);
		for (int i = 0; i < cChainLength; ++i)
		{
			position += Vec3(2.0f * half_cylinder_height, 0, 0);

			Body &segment = *mBodyInterface->CreateBody(BodyCreationSettings(new CapsuleShape(half_cylinder_height, 1), position, Quat::sRotation(Vec3::sAxisX(), 0.25f * ACPH_PI * i) * rotation, i == 0? EMotionType::Static : EMotionType::Dynamic, i == 0? Layers::NON_MOVING : Layers::MOVING));
