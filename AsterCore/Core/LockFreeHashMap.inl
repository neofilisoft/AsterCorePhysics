...
{
	ACPH_ASSERT(mObjectStore == nullptr);

	mObjectStoreSizeBytes = inObjectStoreSizeBytes;
	mObjectStore = reinterpret_cast<uint8 *>(ACPH::AlignedAllocate(inObjectStoreSizeBytes, 16));
}
...
inline uint32 LFHMAllocator::ToOffset(const T *inData) const
{
	...
	ACPH_ASSERT(data >= mObjectStore && data < mObjectStore + mObjectStoreSizeBytes);
...
{
	ACPH_ASSERT(inOffset < mObjectStoreSizeBytes);
...
{
	ACPH_ASSERT(inMaxBuckets >= 4 && IsPowerOf2(inMaxBuckets));
	ACPH_ASSERT(mBuckets == nullptr);
...
void LockFreeHashMap<Key, Value>::Clear()
{
	...
	ACPH_ASSERT(IsAligned(start, 16));
	while (start < end)
	{
...
{
	ACPH_ASSERT(mNumKeyValues == 0);
	ACPH_ASSERT(inNumBuckets <= mMaxBuckets);
	ACPH_ASSERT(inNumBuckets >= 4 && IsPowerOf2(inNumBuckets));
...
inline typename LockFreeHashMap<Key, Value>::KeyValue *LockFreeHashMap<Key, Value>::Create(LFHMAllocatorContext &ioContext, const Key &inKey, uint64 inKeyHash, int inExtraBytes, Params &&... inConstructorParams)
{
	...
	ACPH_ASSERT(Find(inKey, inKeyHash) == nullptr);
	...
	ACPH_ASSERT(intptr_t(kv) % alignof(KeyValue) == 0);
#ifdef ACPH_DEBUG
	memset(kv, 0xcd, size);
#endif
	kv->mKey = inKey;
...
{
	ACPH_ASSERT(mOffset != cInvalidHandle);
...
{
	ACPH_ASSERT(mBucket < mMap->mNumBuckets);
