...
FixedSizeFreeList<Object>::~FixedSizeFreeList()
{
	...
	if (mPages != nullptr)
	{
		...
		ACPH_ASSERT(mNumFreeObjects.load(memory_order_relaxed) == mNumPages * mPageSize);
...
{
	// Check sanity
	ACPH_ASSERT(inPageSize > 0 && IsPowerOf2(inPageSize));
	ACPH_ASSERT(mPages == nullptr);
	...
	ACPH_IF_ENABLE_ASSERTS(mNumFreeObjects = mNumPages * inPageSize;)
		...
		if (first_free == cInvalidObjectIndex)
		{
			...
			if (first_free >= mNumObjectsAllocated)
			{
				...
				while (first_free >= mNumObjectsAllocated)
				{
					...
					mPages[next_page] = reinterpret_cast<ObjectStorage *>(AlignedAllocate(mPageSize * sizeof(ObjectStorage), max<size_t>(alignof(ObjectStorage), ACPH_CACHE_LINE_SIZE)));
					mNumObjectsAllocated += mPageSize;
				}
			}

			// Allocation successful
			ACPH_IF_ENABLE_ASSERTS(mNumFreeObjects.fetch_sub(1, memory_order_relaxed);)
...
{
	ACPH_ASSERT(ioBatch.mNumObjects != uint32(-1), "Trying to reuse a batch that has already been freed");

	// Reset next index
	atomic<uint32> &next_free_object = GetStorage(inObjectIndex).mNextFreeObject;
	ACPH_ASSERT(next_free_object.load(memory_order_relaxed) == inObjectIndex, "Trying to add a object to the batch that is already in a free list");
...
void FixedSizeFreeList<Object>::DestructObjectBatch(Batch &ioBatch)
{
	if (ioBatch.mFirstObjectIndex != cInvalidObjectIndex)
	{
		...
		for (;;)
		{
			...
			{
				// Free successful
				ACPH_IF_ENABLE_ASSERTS(mNumFreeObjects.fetch_add(ioBatch.mNumObjects, memory_order_relaxed);)
...
{
	ACPH_ASSERT(inObjectIndex != cInvalidObjectIndex);
	...
	for (;;)
	{
		...
		{
			// Free successful
			ACPH_IF_ENABLE_ASSERTS(mNumFreeObjects.fetch_add(1, memory_order_relaxed);)
			return;
		}
	}
}
...
inline void FixedSizeFreeList<Object>::DestructObject(Object *inObject)
{
	...
	ACPH_ASSERT(index < mNumObjectsAllocated);
