...
public:
	...
	static constexpr bool	needs_aligned_allocate = alignof(T) > (ACPH_CPU_ADDRESS_BITS == 32? 8 : 16);
	...
	{
		ACPH_ASSERT(inNewSize > 0); // Reallocating to zero size is implementation dependent, so we don't allow it
...
#if (!defined(ACPH_PLATFORM_WINDOWS) || defined(ACPH_COMPILER_MINGW)) && !defined(ACPH_DISABLE_CUSTOM_ALLOCATOR)

namespace std
{
	/// Declare std::hash for String, for some reason on Linux based platforms template deduction takes the wrong variant
	template <>
	struct hash<ACPH::String>
	{
		inline size_t operator () (const ACPH::String &inRHS) const
		{
...
#endif // (!ACPH_PLATFORM_WINDOWS || ACPH_COMPILER_MINGW) && !ACPH_DISABLE_CUSTOM_ALLOCATOR
