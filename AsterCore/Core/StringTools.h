...
ACPH_EXPORT String StringFormat(const char *inFMT, ...);
...
ACPH_EXPORT void StringReplace(String &ioString, const string_view &inSearch, const string_view &inReplace);

/// Convert a delimited string to an array of strings
ACPH_EXPORT void StringToVector(const string_view &inString, Array<String> &outVector, const string_view &inDelimiter = ",", bool inClearVector = true);

/// Convert an array strings to a delimited string
ACPH_EXPORT void VectorToString(const Array<String> &inVector, String &outString, const string_view &inDelimiter = ",");

/// Convert a string to lower case
ACPH_EXPORT String ToLower(const string_view &inString);

/// Converts the lower 4 bits of inNibble to a string that represents the number in binary format
ACPH_EXPORT const char *NibbleToBinary(uint32 inNibble);
