...
public:
	...
	inline					~RefTarget()									{ ACPH_IF_ENABLE_ASSERTS(uint32 value = mRefCount.load(memory_order_relaxed);) ACPH_ASSERT(value == 0 || value == cEmbedded); } ///< assert no one is referencing us

	/// Mark this class as embedded, this means the type can be used in a compound or constructed on the stack.
	/// The Release function will never destruct the object, it is assumed the destructor will be called by whoever allocated
	/// the object and at that point in time it is checked that no references are left to the structure.
	inline void				SetEmbedded() const								{ ACPH_IF_ENABLE_ASSERTS(uint32 old = ) mRefCount.fetch_add(cEmbedded, memory_order_relaxed); ACPH_ASSERT(old < cEmbedded); }
...
class ACPH_EXPORT RefTargetVirtual
{
public:
	/// Virtual destructor
...
namespace std
{
	...
	template <class T>
	struct hash<ACPH::Ref<T>>
	{
		size_t operator () (const ACPH::Ref<T> &inRHS) const
		{
	...
	template <class T>
	struct hash<ACPH::RefConst<T>>
	{
		size_t operator () (const ACPH::RefConst<T> &inRHS) const
		{
			return size_t(inRHS.GetHash());
		}
	};
