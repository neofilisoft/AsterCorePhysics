...
{
	ACPH_IF_ENABLE_ASSERTS(uint32 old_value =) mNumDependencies.fetch_add(inCount, memory_order_relaxed);
	ACPH_ASSERT(old_value > 0 && old_value != cExecutingState && old_value != cDoneState, "Job is queued, running or done, it is not allowed to add a dependency to a running job");
}

bool JobSystem::Job::RemoveDependency(int inCount)
{
	uint32 old_value = mNumDependencies.fetch_sub(inCount, memory_order_release);
	ACPH_ASSERT(old_value != cExecutingState && old_value != cDoneState, "Job is running or done, it is not allowed to add a dependency to a running job");
	uint32 new_value = old_value - inCount;
	ACPH_ASSERT(old_value > new_value, "Test wrap around, this is a logic error");
	return new_value == 0;
}
...
void JobSystem::JobHandle::sRemoveDependencies(const JobHandle *inHandles, uint inNumHandles, int inCount)
{
	...
	Job **jobs_to_queue = (Job **)ACPH_STACK_ALLOC(inNumHandles * sizeof(Job *));
	...
	for (const JobHandle *handle = inHandles, *handle_end = inHandles + inNumHandles; handle < handle_end; ++handle)
	{
		...
		ACPH_ASSERT(job->GetJobSystem() == job_system); // All jobs should belong to the same job system
