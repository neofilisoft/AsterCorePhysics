...
class ACPH_EXPORT TempAllocator : public NonCopyable
{
public:
	...
	/// Allocates inSize bytes of memory, returned memory address must be ACPH_RVECTOR_ALIGNMENT byte aligned
...
class ACPH_EXPORT TempAllocatorImpl final : public TempAllocator
{
public:
	ACPH_OVERRIDE_NEW_DELETE
	...
	explicit						TempAllocatorImpl(size_t inSize) :
		mBase(static_cast<uint8 *>(AlignedAllocate(inSize, ACPH_RVECTOR_ALIGNMENT))),
		mSize(inSize)
	{
	}
	...
	virtual void *					Allocate(uint inSize) override
	{
		...
		{
			size_t new_top = mTop + AlignUp(inSize, ACPH_RVECTOR_ALIGNMENT);
			if (new_top > mSize)
			{
	...
	virtual void					Free(void *inAddress, uint inSize) override
	{
		...
		{
			mTop -= AlignUp(inSize, ACPH_RVECTOR_ALIGNMENT);
	...
	{
		return mTop + AlignUp(inSize, ACPH_RVECTOR_ALIGNMENT) <= mSize;
	}
...
class ACPH_EXPORT TempAllocatorMalloc final : public TempAllocator
{
public:
	ACPH_OVERRIDE_NEW_DELETE
	...
	virtual void *					Allocate(uint inSize) override
	{
		return inSize > 0? AlignedAllocate(inSize, ACPH_RVECTOR_ALIGNMENT) : nullptr;
	}
...
class ACPH_EXPORT TempAllocatorImplWithMallocFallback final : public TempAllocator
{
public:
	ACPH_OVERRIDE_NEW_DELETE
