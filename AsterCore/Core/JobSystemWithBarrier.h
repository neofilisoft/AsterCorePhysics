...
class ACPH_EXPORT JobSystemWithBarrier : public JobSystem
{
public:
...
private:
	...
	protected:
		...
		alignas(ACPH_CACHE_LINE_SIZE) atomic<uint> mJobReadIndex { 0 };		///< First job that could be valid (modulo cMaxJobs), can be nullptr if other thread is still working on adding the job
		alignas(ACPH_CACHE_LINE_SIZE) atomic<uint> mJobWriteIndex { 0 };		///< First job that can be written (modulo cMaxJobs)
