...
/// only intended to wait on the completion of the AsterCore jobs added to it, if you scheduled any jobs in your engine's job system to execute the AsterCore jobs as part of QueueJob/QueueJobs, you might still need
...
class ACPH_EXPORT JobSystem : public NonCopyable
{
protected:
	class Job;
...
protected:
	...
	public:
		...
		/// Constructor
			...
			mJobName(inJobName),
			mColor(inColor),
		#endif // defined(ACPH_EXTERNAL_PROFILE) || defined(ACPH_PROFILE_ENABLED)
		...
		inline bool			SetBarrier(Barrier *inBarrier)
		{
			...
			if (mBarrier.compare_exchange_strong(barrier, reinterpret_cast<intptr_t>(inBarrier), memory_order_relaxed))
				return true;
			ACPH_ASSERT(barrier == cBarrierDoneState, "A job can only belong to 1 barrier");
			return false;
		}
		...
		inline uint32		Execute()
		{
			...
			{
				ACPH_PROFILE(mJobName, mColor.GetUInt32());
				mJobFunction();
			}
