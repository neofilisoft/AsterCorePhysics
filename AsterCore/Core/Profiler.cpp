...
#if defined(ACPH_EXTERNAL_PROFILE) && defined(ACPH_SHARED_LIBRARY)
...
void Profiler::sAggregate(int inDepth, uint32 inColor, ProfileSample *&ioSample, const ProfileSample *inEnd, Aggregators &ioAggregators, KeyToAggregator &ioKeyToAggregator)
{
	...
	{
		ACPH_ASSERT(sample[-1].mStartCycle <= sample->mStartCycle);
		ACPH_ASSERT(sample->mStartCycle >= ioSample->mStartCycle);
		ACPH_ASSERT(sample->mEndCycle <= ioSample->mEndCycle);
	...
	ACPH_ASSERT(sample[-1].mStartCycle <= ioSample->mEndCycle);
	ACPH_ASSERT(sample >= inEnd || sample->mStartCycle >= ioSample->mEndCycle);
	ioSample = sample - 1;
}
