...
class ACPH_EXPORT_GCC_BUG_WORKAROUND [[nodiscard]] Color
{
public:
	/// Constructors
	...
	inline uint8			operator () (uint inIdx) const											{ ACPH_ASSERT(inIdx < 4); return (&r)[inIdx]; }
	inline uint8 &			operator () (uint inIdx)												{ ACPH_ASSERT(inIdx < 4); return (&r)[inIdx]; }
