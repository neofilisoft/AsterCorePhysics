...
#include <AsterCore/Core/STLAllocator.h>
#include <AsterCore/Core/HashCombine.h>

#ifdef ACPH_USE_STD_VECTOR
...
private:
	...
	{
		ACPH_ASSERT(inNewCapacity > 0 && inNewCapacity >= mSize);
...
public:
	...
	{
		ACPH_ASSERT(&inValue < mElements || &inValue >= mElements + mSize, "Can't pass an element from the array to resize");
...
public:
	...
	{
		ACPH_ASSERT(&inValue < mElements || &inValue >= mElements + mSize, "Can't pass an element from the array to push_back");
	...
	{
		ACPH_ASSERT(&inValue < mElements || &inValue >= mElements + mSize, "Can't pass an element from the array to insert");
...
namespace std
{
	...
	struct hash<ACPH::Array<T, Allocator>>
	{
		size_t operator () (const ACPH::Array<T, Allocator> &inRHS) const
		{
