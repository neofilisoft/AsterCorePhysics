...
#define ACPH_DEFINE_TRIVIAL_HASH(type)						\
template <>													\
struct Hash<type>											\
{															\
...
/// Commonly used types
ACPH_DEFINE_TRIVIAL_HASH(char)
ACPH_DEFINE_TRIVIAL_HASH(int)
ACPH_DEFINE_TRIVIAL_HASH(uint32)
ACPH_DEFINE_TRIVIAL_HASH(uint64)
...
///		};
///
///		ACPH_MAKE_HASHABLE(SomeHashKey, t.key1, t.key2, t.key3)
...
#define ACPH_MAKE_HASH_STRUCT(type, name, ...)				\
	struct [[nodiscard]] name								\
	{														\
		::ACPH::uint64 operator()(const type &t) const		\
		{													\
			return ::ACPH::HashCombineArgs(__VA_ARGS__);		\
		}													\
	};

#define ACPH_MAKE_STD_HASH(type)								\
	...
	{														\
		...
		{													\
			...
			{												\
				return size_t(::ACPH::Hash<type>{ }(t));		\
			}												\
		};													\
	}														\
	ACPH_SUPPRESS_WARNING_POP

#define ACPH_MAKE_HASHABLE(type, ...)						\
	ACPH_SUPPRESS_WARNING_PUSH								\
	ACPH_SUPPRESS_WARNINGS									\
	namespace ACPH											\
	{														\
		template<>											\
		ACPH_MAKE_HASH_STRUCT(type, Hash<type>, __VA_ARGS__) \
	}														\
	ACPH_SUPPRESS_WARNING_POP								\
	ACPH_MAKE_STD_HASH(type)
