...
#include <AsterCore/Geometry/RayTriangle.h>
...
public:
	...
	public:
		...
		void						FinalizePreparePack(uint64 &ioBufferSize)
		{
			...
			ACPH_ASSERT(IsAligned(ioBufferSize, 4));
		...
		size_t						Pack(const IndexedTriangle *inTriangles, uint inNumTriangles, bool inStoreUserData, ByteBuffer &ioBuffer, const char *&outError)
		{
			...
			if (inStoreUserData)
			{
				...
				ACPH_ASSERT(num_blocks <= OFFSET_TO_USERDATA_MASK);
		...
		void						Finalize(const VertexList &inVertices, TriangleHeader *ioHeader, ByteBuffer &ioBuffer) const
		{
			...
			ACPH_ASSERT(mVertices.size() == mVertexCount);
			ACPH_ASSERT(ioBuffer.size() == mVerticesStartIdx);
			...
			for (uint32 v : mVertices)
			{
				...
				ACPH_ASSERT(c.GetX() <= COMPONENT_MASK);
				ACPH_ASSERT(c.GetY() <= COMPONENT_MASK);
				ACPH_ASSERT(c.GetZ() <= COMPONENT_MASK);
	...
	private:
		...
		ACPH_INLINE void				Unpack(const VertexData *inVertices, UVec4Arg inIndex, Vec4 &outX, Vec4 &outY, Vec4 &outZ) const
		{
			// Get compressed data
		...
		ACPH_INLINE void				Unpack(const TriangleBlock *inBlock, const VertexData *inVertices, Vec4 &outX1, Vec4 &outY1, Vec4 &outZ1, Vec4 &outX2, Vec4 &outY2, Vec4 &outZ2, Vec4 &outX3, Vec4 &outY3, Vec4 &outZ3) const
		{
	...
	public:
		ACPH_INLINE explicit			DecodingContext(const TriangleHeader *inHeader) :
		...
		ACPH_INLINE void				Unpack(const void *inTriangleStart, uint32 inNumTriangles, Vec3 *outTriangles) const
		{
			ACPH_ASSERT(inNumTriangles > 0);
		...
		ACPH_INLINE float			TestRay(Vec3Arg inRayOrigin, Vec3Arg inRayDirection, const void *inTriangleStart, uint32 inNumTriangles, float inClosest, uint32 &outClosestTriangleIndex) const
		{
			ACPH_ASSERT(inNumTriangles > 0);
			const TriangleBlockHeader *header = reinterpret_cast<const TriangleBlockHeader *>(inTriangleStart);
			const VertexData *vertices = header->GetVertexData();
		...
		ACPH_INLINE uint32			GetUserData(const void *inTriangleStart, uint32 inTriangleIdx) const
		{
			const TriangleBlockHeader *header = reinterpret_cast<const TriangleBlockHeader *>(inTriangleStart);
		...
		ACPH_INLINE static void		sGetFlags(const void *inTriangleStart, uint32 inNumTriangles, uint8 *outTriangleFlags)
		{
			ACPH_ASSERT(inNumTriangles > 0);
			const TriangleBlockHeader *header = reinterpret_cast<const TriangleBlockHeader *>(inTriangleStart);
			const TriangleBlock *t = header->GetTriangleBlock();
		...
		ACPH_INLINE static uint8		sGetFlags(const void *inTriangleStart, int inTriangleIndex)
		{
			const TriangleBlockHeader *header = reinterpret_cast<const TriangleBlockHeader *>(inTriangleStart);
		...
		ACPH_INLINE void				Unpack(const void *inTriangleStart, uint32 inNumTriangles, Vec3 *outTriangles, uint8 *outTriangleFlags) const
		{
