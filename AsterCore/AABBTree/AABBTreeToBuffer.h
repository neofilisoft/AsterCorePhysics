...
#include <AsterCore/AABBTree/AABBTreeBuilder.h>
	...
	bool							Convert(const Array<IndexedTriangle> &inTriangles, const Array<AABBTreeBuilder::Node> &inNodes, const VertexList &inVertices, const AABBTreeBuilder::Node *inRoot, bool inStoreUserData, const char *&outError)
	{
		...
		for (;;)
		{
			while (!to_process.empty())
			{
				...
				ACPH_ASSERT(AABox(node_data->mNodeBoundsMin, node_data->mNodeBoundsMax).Contains(node_data->mNode->mBounds), "AABBTreeToBuffer: Bounding box became smaller!");
				...
				if (node_data->mNode->HasChildren())
				{
					...
					for (int idx = int(child_nodes.size()) - 1; idx >= 0; --idx)
					{
						...
						// Due to quantization box could have become bigger, not smaller
						ACPH_ASSERT(AABox(child_bounds_min[idx], child_bounds_max[idx]).Contains(child_node->mBounds), "AABBTreeToBuffer: Bounding box became smaller!");
		...
		ACPH_ASSERT(to_process_max_size == to_process.capacity() || to_process_triangles_max_size == to_process.capacity());
		ACPH_ASSERT(to_process_max_size == to_process_triangles.capacity() || to_process_triangles_max_size == to_process_triangles.capacity());
