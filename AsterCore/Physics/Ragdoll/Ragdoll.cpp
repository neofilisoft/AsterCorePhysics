...
#include <Aster/AsterCore.h>

#include <Aster/Physics/Ragdoll/Ragdoll.h>
#include <Aster/Physics/Constraints/SwingTwistConstraint.h>
#include <Aster/Physics/Constraints/HingeConstraint.h>
#include <Aster/Physics/PhysicsSystem.h>
#include <Aster/Physics/Body/BodyLockMulti.h>
#include <Aster/Physics/Collision/GroupFilterTable.h>
#include <Aster/Physics/Collision/CollisionCollectorImpl.h>
#include <Aster/Physics/Collision/CollideShape.h>
#include <Aster/Physics/Collision/CollisionDispatch.h>
#include <Aster/ObjectStream/TypeDeclarations.h>
#include <Aster/Core/StreamIn.h>
#include <Aster/Core/StreamOut.h>

ACPH_NAMESPACE_BEGIN

ACPH_IMPLEMENT_SERIALIZABLE_NON_VIRTUAL(RagdollSettings::Part)
{
	ACPH_ADD_BASE_CLASS(RagdollSettings::Part, BodyCreationSettings)

	ACPH_ADD_ATTRIBUTE(RagdollSettings::Part, mToParent)
}

ACPH_IMPLEMENT_SERIALIZABLE_NON_VIRTUAL(RagdollSettings::AdditionalConstraint)
{
	ACPH_ADD_ATTRIBUTE(RagdollSettings::AdditionalConstraint, mBodyIdx)
	ACPH_ADD_ATTRIBUTE(RagdollSettings::AdditionalConstraint, mConstraint)
}

ACPH_IMPLEMENT_SERIALIZABLE_NON_VIRTUAL(RagdollSettings)
{
	ACPH_ADD_ATTRIBUTE(RagdollSettings, mSkeleton)
	ACPH_ADD_ATTRIBUTE(RagdollSettings, mParts)
	ACPH_ADD_ATTRIBUTE(RagdollSettings, mAdditionalConstraints)
}
	...
	for (int first_idx = 0; first_idx < mSkeleton->GetJointCount(); ++first_idx)
	{
		...
		if (!visited[first_idx] && (first_idx_parent == -1 || visited[first_idx_parent]))
		{
			...
			for (int i : indices)
				...
				{
					ACPH_ASSERT(false, "Failed to decompose the inertia tensor!");
					return false;
				}
...
void RagdollSettings::DisableParentChildCollisions(const Mat44 *inJointMatrices, float inMinSeparationDistance)
{
	...
	ACPH_ASSERT(joint_count == (int)mParts.size());
...
Ragdoll *RagdollSettings::CreateRagdoll(CollisionGroup::GroupID inCollisionGroup, uint64 inUserData, PhysicsSystem *inSystem) const
{
	...
	Body **bodies = (Body **)ACPH_STACK_ALLOC(mParts.size() * sizeof(Body *));
	int joint_idx = 0;
	for (const Part &p : mParts)
	{
...
{
	// Scope for ACPH_STACK_ALLOC
	{
		// Create copy of body ids since they will be shuffled
		int num_bodies = (int)mBodyIDs.size();
		BodyID *bodies = (BodyID *)ACPH_STACK_ALLOC(num_bodies * sizeof(BodyID));
...
{
	ACPH_ASSERT(inPose.GetSkeleton() == mRagdollSettings->mSkeleton);
...
{
	ACPH_ASSERT(outPose.GetSkeleton() == mRagdollSettings->mSkeleton);
	...
	for (int i = 0; i < (int)inPose.GetJointMatrices().size(); ++i)
	{
		...
		if (constraint_idx >= 0)
		{
			...
			else if (sub_type == EConstraintSubType::Hinge)
			{
				...
				ACPH_ASSERT(false, "Constraint type not implemented!");
		}
	}
}
