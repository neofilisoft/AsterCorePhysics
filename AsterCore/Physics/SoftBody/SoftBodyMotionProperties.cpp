...
#include <AsterCore/Physics/Collision/SimShapeFilterWrapper.h>
#include <AsterCore/Physics/PhysicsSystem.h>
#include <AsterCore/Physics/Body/BodyManager.h>
#include <AsterCore/Core/ScopeExit.h>
...
void SoftBodyMotionProperties::ApplyLRAConstraints(uint inStartIndex, uint inEndIndex)
{
	...
	{
		ACPH_ASSERT(lra->mVertex[0] < mVertices.size());
		ACPH_ASSERT(lra->mVertex[1] < mVertices.size());
...
void SoftBodyMotionProperties::UpdateSoftBodyState(SoftBodyUpdateContext &ioContext, const PhysicsSettings &inPhysicsSettings)
{
	...
	if (mUpdatePosition)
	{
		...
		for (SkinState &s : mSkinState)
			s.mPosition -= delta;
		ACPH_IF_DEBUG_RENDERER(mSkinStateTransform.SetTranslation(mSkinStateTransform.GetTranslation() + ioContext.mDeltaPosition);)
...
void SoftBodyMotionProperties::StartFirstIteration(SoftBodyUpdateContext &ioContext)
{
	...
	ACPH_IF_ENABLE_ASSERTS(uint iteration =) ioContext.mNextIteration.fetch_add(1, memory_order_relaxed);
	ACPH_ASSERT(iteration == 0);
...
SoftBodyMotionProperties::EStatus SoftBodyMotionProperties::ParallelApplyConstraints(SoftBodyUpdateContext &ioContext, const PhysicsSettings &inPhysicsSettings)
{
	...
	ACPH_ASSERT(num_groups > 0, "SoftBodySharedSettings::Optimize should have been called!");
...
void SoftBodyMotionProperties::SkinVertices([[maybe_unused]] RMat44Arg inCenterOfMassTransform, const Mat44 *inJointMatrices, [[maybe_unused]] uint inNumJoints, bool inHardSkinAll, TempAllocator &ioTempAllocator)
{
	...
	ACPH_SCOPE_EXIT([&ioTempAllocator, skin_matrices, skin_matrices_size]{ ioTempAllocator.Free(skin_matrices, skin_matrices_size); });
	...
	{
		ACPH_ASSERT(inv_bind_matrix->mJointIndex < inNumJoints);
		*s = inJointMatrices[inv_bind_matrix->mJointIndex] * inv_bind_matrix->mInvBind;
	}

	// Skin the vertices
	ACPH_IF_DEBUG_RENDERER(mSkinStateTransform = inCenterOfMassTransform;)
	ACPH_IF_ENABLE_ASSERTS(uint num_vertices = uint(mSettings->mVertices.size());)
	ACPH_ASSERT(mSkinState.size() == num_vertices);
	...
	{
		// Get bind pose
		ACPH_ASSERT(s.mVertex < num_vertices);
		...
		for (const SkinWeight &w : s.mWeights)
		{
			...
			ACPH_ASSERT(w.mInvBindIndex < num_skin_matrices);
...
void SoftBodyMotionProperties::CustomUpdate(float inDeltaTime, Body &ioSoftBody, PhysicsSystem &inSystem)
{
	...
	while ((status = ParallelUpdate(context, settings)) == EStatus::DidWork)
		continue;
	ACPH_ASSERT(status == EStatus::Done);
