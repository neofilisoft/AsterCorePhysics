...
namespace
{
	...
	static uint64 HashCoordinate(const SoftBodyCellCoordinate &inCoordinate)
	{
...
bool SoftBodyAdvancedSimulation::sShouldFractureEdge(float inRestLength, float inCurrentLength, const SoftBodyFractureSettings &inSettings)
{
...
void SoftBodyAdvancedSimulation::sBuildSelfCollisionPairs(const Vec3 *inPositions, uint32 inVertexCount, const SoftBodySelfCollisionSettings &inSettings, Array<SoftBodySelfCollisionPair> &outPairs)
{
	outPairs.clear();
	if (!inSettings.mEnableSelfCollision || inPositions == nullptr || inVertexCount < 2)
		return;

	const float cell_size = max(inSettings.mThickness, 1.0e-4f);
	...
	for (uint32 i = 0; i < inVertexCount; ++i)
	{
		...
		const SoftBodyCellCoordinate coordinate
		{
	...
	{
		const Vec3 position = inPositions[i];
		const SoftBodyCellCoordinate center
		{
			int(std::floor(position.GetX() / cell_size)),
			int(std::floor(position.GetY() / cell_size)),
			int(std::floor(position.GetZ() / cell_size))
