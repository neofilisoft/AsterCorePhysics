...
public:
	...
	ACPH_INLINE bool					HasContact(const SoftBodyVertex &inVertex) const
	{
	...
	ACPH_INLINE Vec3					GetLocalContactPoint(const SoftBodyVertex &inVertex) const
	{
	...
	ACPH_INLINE Vec3					GetContactNormal(const SoftBodyVertex &inVertex) const
	{
		return -inVertex.mCollisionPlane.GetNormal();
	}

	/// Get the body with which the vertex has collided in this update
	ACPH_INLINE BodyID				GetContactBodyID(const SoftBodyVertex &inVertex) const
	{
	...
	ACPH_INLINE uint					GetNumSensorContacts() const
	{
		return (uint)mCollidingSensors.size();
	}

	/// Get the i-th sensor that is in contact with the soft body
	ACPH_INLINE BodyID				GetSensorContactBodyID(uint inIndex) const
	{
