...
#include <AsterCore/Physics/Collision/CastConvexVsTriangles.h>
#include <AsterCore/Physics/Collision/CastSphereVsTriangles.h>
#include <AsterCore/Physics/Collision/CollideConvexVsTriangles.h>
#include <AsterCore/Physics/Collision/CollideSphereVsTriangles.h>
...
void SoftBodyShape::GetSubmergedVolume(Mat44Arg inCenterOfMassTransform, Vec3Arg inScale, const Plane &inSurface, float &outTotalVolume, float &outSubmergedVolume, Vec3 &outCenterOfBuoyancy ACPH_IF_DEBUG_RENDERER(, RVec3Arg inBaseOffset)) const
{
...
{
	ACPH_ASSERT(inShape1->GetType() == EShapeType::Convex);
	const ConvexShape *shape1 = static_cast<const ConvexShape *>(inShape1);
	ACPH_ASSERT(inShape2->GetSubType() == EShapeSubType::SoftBody);
...
{
	ACPH_ASSERT(inShape1->GetSubType() == EShapeSubType::Sphere);
...
{
	ACPH_ASSERT(inShape->GetSubType() == EShapeSubType::SoftBody);
