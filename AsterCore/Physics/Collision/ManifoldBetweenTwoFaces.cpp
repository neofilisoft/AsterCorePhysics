...
#include <AsterCore/Geometry/ClipPoly.h>
...
void PruneContactPoints(Vec3Arg inPenetrationAxis, ContactPoints &ioContactPointsOn1, ContactPoints &ioContactPointsOn2 ACPH_IF_DEBUG_RENDERER(, RVec3Arg inCenterOfMass))
{
	// Makes no sense to call this with 4 or less points
	ACPH_ASSERT(ioContactPointsOn1.size() > 4);

	// Both arrays should have the same size
	ACPH_ASSERT(ioContactPointsOn1.size() == ioContactPointsOn2.size());

	// Penetration axis must be normalized
	ACPH_ASSERT(inPenetrationAxis.IsNormalized());
