...
class ACPH_EXPORT TransformedShape
{
public:
	...
	inline SubShapeID			MakeSubShapeIDRelativeToShape(const SubShapeID &inSubShapeID) const
	{
		...
		ACPH_IF_ENABLE_ASSERTS(uint32 root_id =) inSubShapeID.PopID(num_bits_written, sub_shape_id);
		ACPH_ASSERT(root_id == (mSubShapeIDCreator.GetID().GetValue() & ((1 << num_bits_written) - 1)));
		return sub_shape_id;
	}
...
static_assert(ACPH_CPU_ADDRESS_BITS != 64 || ACPH_RVECTOR_ALIGNMENT < 16 || sizeof(TransformedShape) == ACPH_IF_SINGLE_PRECISION_ELSE(64, 96), "Not properly packed");
static_assert(alignof(TransformedShape) == max(ACPH_VECTOR_ALIGNMENT, ACPH_RVECTOR_ALIGNMENT), "Not properly aligned");
