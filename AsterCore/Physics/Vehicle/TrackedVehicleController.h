...
class ACPH_EXPORT WheelSettingsTV : public WheelSettings
{
	ACPH_DECLARE_SERIALIZABLE_VIRTUAL(ACPH_EXPORT, WheelSettingsTV)
...
class ACPH_EXPORT WheelTV : public Wheel
{
public:
...
class ACPH_EXPORT TrackedVehicleControllerSettings : public VehicleControllerSettings
{
	ACPH_DECLARE_SERIALIZABLE_VIRTUAL(ACPH_EXPORT, TrackedVehicleControllerSettings)
...
class ACPH_EXPORT TrackedVehicleController : public VehicleController
{
public:
	ACPH_OVERRIDE_NEW_DELETE
	...
	void						SetDriverInput(float inForward, float inLeftRatio, float inRightRatio, float inBrake) { ACPH_ASSERT(inLeftRatio != 0.0f && inRightRatio != 0.0f); mForwardInput = inForward; mLeftRatio = inLeftRatio; mRightRatio = inRightRatio; mBrakeInput = inBrake; }
	...
	void						SetLeftRatio(float inLeftRatio)				{ ACPH_ASSERT(inLeftRatio != 0.0f); mLeftRatio = inLeftRatio; }
	...
	void						SetRightRatio(float inRightRatio)			{ ACPH_ASSERT(inRightRatio != 0.0f); mRightRatio = inRightRatio; }
...
protected:
	...
	// See: VehicleController
	virtual Wheel *				ConstructWheel(const WheelSettings &inWheel) const override { ACPH_ASSERT(IsKindOf(&inWheel, ACPH_RTTI(WheelSettingsTV))); return new WheelTV(static_cast<const WheelSettingsTV &>(inWheel)); }
