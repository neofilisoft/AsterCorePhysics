...
#include <AsterCore/Physics/Vehicle/WheeledVehicleController.h>
...
#include <AsterCore/Math/DynMatrix.h>
#include <AsterCore/Math/GaussianElimination.h>
#ifdef ACPH_DEBUG_RENDERER
	#include <AsterCore/Renderer/DebugRenderer.h>
#endif // ACPH_DEBUG_RENDERER

//#define ACPH_TRACE_VEHICLE_STATS

ACPH_NAMESPACE_BEGIN

ACPH_IMPLEMENT_SERIALIZABLE_VIRTUAL(WheeledVehicleControllerSettings)
{
	ACPH_ADD_BASE_CLASS(WheeledVehicleControllerSettings, VehicleControllerSettings)

	ACPH_ADD_ATTRIBUTE(WheeledVehicleControllerSettings, mEngine)
	ACPH_ADD_ATTRIBUTE(WheeledVehicleControllerSettings, mTransmission)
	ACPH_ADD_ATTRIBUTE(WheeledVehicleControllerSettings, mDifferentials)
	ACPH_ADD_ATTRIBUTE(WheeledVehicleControllerSettings, mDifferentialLimitedSlipRatio)
}

ACPH_IMPLEMENT_SERIALIZABLE_VIRTUAL(WheelSettingsWV)
{
	ACPH_ADD_BASE_CLASS(WheelSettingsWV, WheelSettings)

	ACPH_ADD_ATTRIBUTE(WheelSettingsWV, mInertia)
	ACPH_ADD_ATTRIBUTE(WheelSettingsWV, mAngularDamping)
	ACPH_ADD_ATTRIBUTE(WheelSettingsWV, mMaxSteerAngle)
	ACPH_ADD_ATTRIBUTE(WheelSettingsWV, mLongitudinalFriction)
	ACPH_ADD_ATTRIBUTE(WheelSettingsWV, mLateralFriction)
	ACPH_ADD_ATTRIBUTE(WheelSettingsWV, mMaxBrakeTorque)
	ACPH_ADD_ATTRIBUTE(WheelSettingsWV, mMaxHandBrakeTorque)
}
...
WheelWV::WheelWV(const WheelSettingsWV &inSettings) :
	Wheel(inSettings)
{
	ACPH_ASSERT(inSettings.mInertia >= 0.0f);
	ACPH_ASSERT(inSettings.mAngularDamping >= 0.0f);
	ACPH_ASSERT(abs(inSettings.mMaxSteerAngle) <= 0.5f * ACPH_PI);
	ACPH_ASSERT(inSettings.mMaxBrakeTorque >= 0.0f);
	ACPH_ASSERT(inSettings.mMaxHandBrakeTorque >= 0.0f);
}
...
void WheelWV::Update(uint inWheelIndex, float inDeltaTime, const VehicleConstraint &inConstraint)
{
	...
	mAngle = fmod(mAngle + mAngularVelocity * inDeltaTime, 2.0f * ACPH_PI);
	...
	VehicleController(inConstraint)
{
	...
	ACPH_ASSERT(inSettings.mEngine.mMinRPM >= 0.0f);
	ACPH_ASSERT(inSettings.mEngine.mMinRPM <= inSettings.mEngine.mMaxRPM);
	...
	ACPH_ASSERT(inSettings.mTransmission.mSwitchTime >= 0.0f);
	ACPH_ASSERT(inSettings.mTransmission.mShiftDownRPM > 0.0f);
	ACPH_ASSERT(inSettings.mTransmission.mMode != ETransmissionMode::Auto || inSettings.mTransmission.mShiftUpRPM < inSettings.mEngine.mMaxRPM);
	ACPH_ASSERT(inSettings.mTransmission.mShiftUpRPM > inSettings.mTransmission.mShiftDownRPM);
	ACPH_ASSERT(inSettings.mTransmission.mClutchStrength > 0.0f);
	...
	for (uint i = 0; i < mDifferentials.size(); ++i)
	{
		...
		mDifferentials[i] = d;
		ACPH_ASSERT(d.mDifferentialRatio > 0.0f);
		ACPH_ASSERT(d.mLeftRightSplit >= 0.0f && d.mLeftRightSplit <= 1.0f);
		ACPH_ASSERT(d.mEngineTorqueRatio >= 0.0f);
		ACPH_ASSERT(d.mLimitedSlipRatio > 1.0f);
	}

	mDifferentialLimitedSlipRatio = inSettings.mDifferentialLimitedSlipRatio;
	ACPH_ASSERT(mDifferentialLimitedSlipRatio > 1.0f);
}
...
#ifdef ACPH_TRACE_VEHICLE_STATS
...
#endif // ACPH_TRACE_VEHICLE_STATS
...
void WheeledVehicleController::PostCollide(float inDeltaTime, PhysicsSystem &inPhysicsSystem)
{
	...
	ACPH_ASSERT(abs(sum_torque_factors - 1.0f) < 1.0e-6f);
#endif // ACPH_ENABLE_ASSERTS
	...
	if (!driven_wheels.empty())
	{
		...
		{
			ACPH_ASSERT(false, "New engine/wheel speeds could not be calculated!");
		}
	}
...
bool WheeledVehicleController::SolveLongitudinalAndLateralConstraints(float inDeltaTime)
{
	...
	float *max_lateral_friction_impulse = (float *)ACPH_STACK_ALLOC(mConstraint.GetWheels().size() * sizeof(float));
