...
class ACPH_EXPORT VehicleEngineSettings
{
	ACPH_DECLARE_SERIALIZABLE_NON_VIRTUAL(ACPH_EXPORT, VehicleEngineSettings)
...
class ACPH_EXPORT VehicleEngine : public VehicleEngineSettings
{
public:
	/// Multiply an angular velocity (rad/s) with this value to get rounds per minute (RPM)
	static constexpr float	cAngularVelocityToRPM = 60.0f / (2.0f * ACPH_PI);
	...
	float					ConvertRPMToAngle(float inRPM) const		{ return (-0.75f + 1.5f * inRPM / mMaxRPM) * ACPH_PI; }
