...
class ACPH_EXPORT WheelSettings : public SerializableObject, public RefTarget<WheelSettings>
{
	ACPH_DECLARE_SERIALIZABLE_VIRTUAL(ACPH_EXPORT, WheelSettings)
...
class ACPH_EXPORT Wheel : public NonCopyable
{
public:
	...
	RVec3					GetContactPosition() const					{ ACPH_ASSERT(HasContact()); return mContactPosition; }

	/// Velocity of the contact point (m / s, not relative to the wheel but in world space)
	Vec3					GetContactPointVelocity() const				{ ACPH_ASSERT(HasContact()); return mContactPointVelocity; }

	/// Returns the current contact normal in world space (note by the time you call this the vehicle has moved)
	Vec3					GetContactNormal() const					{ ACPH_ASSERT(HasContact()); return mContactNormal; }
	...
	Vec3					GetContactLongitudinal() const				{ ACPH_ASSERT(HasContact()); return mContactLongitudinal; }

	/// Returns lateral direction (sideways direction) in world space (note by the time you call this the vehicle has moved)
	Vec3					GetContactLateral() const					{ ACPH_ASSERT(HasContact()); return mContactLateral; }
