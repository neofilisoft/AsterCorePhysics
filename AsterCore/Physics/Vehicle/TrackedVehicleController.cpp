...
#include <AsterCore/Physics/Vehicle/TrackedVehicleController.h>
...
ACPH_IMPLEMENT_SERIALIZABLE_VIRTUAL(TrackedVehicleControllerSettings)
{
	ACPH_ADD_BASE_CLASS(TrackedVehicleControllerSettings, VehicleControllerSettings)

	ACPH_ADD_ATTRIBUTE(TrackedVehicleControllerSettings, mEngine)
	ACPH_ADD_ATTRIBUTE(TrackedVehicleControllerSettings, mTransmission)
	ACPH_ADD_ATTRIBUTE(TrackedVehicleControllerSettings, mTracks)
}

ACPH_IMPLEMENT_SERIALIZABLE_VIRTUAL(WheelSettingsTV)
{
	ACPH_ADD_BASE_CLASS(WheelSettingsTV, WheelSettings)

	ACPH_ADD_ATTRIBUTE(WheelSettingsTV, mLongitudinalFriction)
	ACPH_ADD_ATTRIBUTE(WheelSettingsTV, mLateralFriction)
}
	...
	VehicleController(inConstraint)
{
	...
	for (uint i = 0; i < std::size(mTracks); ++i)
	{
		...
		ACPH_ASSERT(d.mInertia >= 0.0f);
		ACPH_ASSERT(d.mAngularDamping >= 0.0f);
		ACPH_ASSERT(d.mMaxBrakeTorque >= 0.0f);
