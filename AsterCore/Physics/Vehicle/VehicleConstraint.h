...
#include <AsterCore/Physics/PhysicsStepListener.h>
#include <AsterCore/Physics/Constraints/ConstraintPart/AngleConstraintPart.h>
#include <AsterCore/Physics/Vehicle/VehicleCollisionTester.h>
...
class ACPH_EXPORT VehicleConstraintSettings : public ConstraintSettings
{
	ACPH_DECLARE_SERIALIZABLE_VIRTUAL(ACPH_EXPORT, VehicleConstraintSettings)
...
public:
	...
	float						mMaxPitchRollAngle = ACPH_PI;				///< Defines the maximum pitch/roll angle (rad), can be used to avoid the car from getting upside down. The vehicle up direction will stay within a cone centered around the up axis with half top angle mMaxPitchRollAngle, set to pi to turn off.
...
///
/// When the vehicle drives over very light objects (rubble) you may see the car body dip down. This is a known issue and is an artifact of the iterative solver that AsterCore is using.
...
class ACPH_EXPORT VehicleConstraint : public Constraint, public PhysicsStepListener
{
public:
