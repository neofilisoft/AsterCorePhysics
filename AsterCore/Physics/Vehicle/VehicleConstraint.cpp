...
ACPH_IMPLEMENT_SERIALIZABLE_VIRTUAL(VehicleConstraintSettings)
{
	ACPH_ADD_BASE_CLASS(VehicleConstraintSettings, ConstraintSettings)

	ACPH_ADD_ATTRIBUTE(VehicleConstraintSettings, mUp)
	ACPH_ADD_ATTRIBUTE(VehicleConstraintSettings, mForward)
	ACPH_ADD_ATTRIBUTE(VehicleConstraintSettings, mMaxPitchRollAngle)
	ACPH_ADD_ATTRIBUTE(VehicleConstraintSettings, mWheels)
	ACPH_ADD_ATTRIBUTE(VehicleConstraintSettings, mAntiRollBars)
	ACPH_ADD_ATTRIBUTE(VehicleConstraintSettings, mController)
}
	...
	mAntiRollBars(inSettings.mAntiRollBars)
{
	...
	ACPH_ASSERT(inSettings.mUp.IsNormalized());
	ACPH_ASSERT(inSettings.mForward.IsNormalized());
	ACPH_ASSERT(!inSettings.mWheels.empty());
...
{
	ACPH_ASSERT(inWheelIndex < mWheels.size());
...
void VehicleConstraint::BuildIslands(uint32 inConstraintIndex, IslandBuilder &ioBuilder, BodyManager &inBodyManager)
{
	...
	BodyID *body_ids = (BodyID *)ACPH_STACK_ALLOC((mWheels.size() + 1) * sizeof(BodyID));
	int num_bodies = 0;
		...
		if (w->mContactBody != nullptr)
		{
			...
			if (settings->mSuspensionMaxLength > settings->mSuspensionMinLength)
			{
				...
				if (settings->mSuspensionSpring.mMode == ESpringMode::FrequencyAndDamping)
				{
					...
					float omega = 2.0f * ACPH_PI * settings->mSuspensionSpring.mFrequency;
