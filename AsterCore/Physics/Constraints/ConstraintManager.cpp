...
#include <AsterCore/Physics/Constraints/ConstraintManager.h>
...
#include <AsterCore/Physics/PhysicsLock.h>
...
{
	UniqueLock lock(mConstraintsMutex ACPH_IF_ENABLE_ASSERTS(, mLockContext, EPhysicsLockTypes::ConstraintsList));
	...
	for (Constraint **c = inConstraints, **c_end = inConstraints + inNumber; c < c_end; ++c)
	{
		...
		ACPH_ASSERT(constraint->mConstraintIndex == Constraint::cInvalidConstraintIndex);
...
void ConstraintManager::Remove(Constraint **inConstraints, int inNumber)
{
	...
	for (Constraint **c = inConstraints, **c_end = inConstraints + inNumber; c < c_end; ++c)
	{
		...
		ACPH_ASSERT(this_constraint_idx != Constraint::cInvalidConstraintIndex);
...
void ConstraintManager::GetActiveConstraints(uint32 inStartConstraintIdx, uint32 inEndConstraintIdx, Constraint **outActiveConstraints, uint32 &outNumActiveConstraints) const
{
	...
	ACPH_ASSERT(inEndConstraintIdx <= mConstraints.size());
	...
	for (uint32 constraint_idx = inStartConstraintIdx; constraint_idx < inEndConstraintIdx; ++constraint_idx)
	{
		...
		ACPH_ASSERT(c->mConstraintIndex == constraint_idx);
		if (c->IsActive())
		{
...
bool ConstraintManager::RestoreState(StateRecorder &inStream)
{
	...
	if (inStream.IsValidating())
	{
		...
		{
			ACPH_ASSERT(false, "Cannot handle adding/removing constraints");
			return false;
		}
		for (const Ref<Constraint> &c : mConstraints)
		{
			...
			{
				ACPH_ASSERT(false, "Unexpected constraint index");
				return false;
			}
	...
	else
	{
		...
		for (uint32 idx = 0; idx < num_constraints; ++idx)
		{
			...
			{
				ACPH_ASSERT(false, "Restoring state for non-existing constraint");
				return false;
			}
