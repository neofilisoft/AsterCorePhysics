...
#include <AsterCore/Physics/Constraints/GearConstraint.h>
...
ACPH_IMPLEMENT_SERIALIZABLE_VIRTUAL(GearConstraintSettings)
{
	ACPH_ADD_BASE_CLASS(GearConstraintSettings, TwoBodyConstraintSettings)

	ACPH_ADD_ENUM_ATTRIBUTE(GearConstraintSettings, mSpace)
	ACPH_ADD_ATTRIBUTE(GearConstraintSettings, mHingeAxis1)
	ACPH_ADD_ATTRIBUTE(GearConstraintSettings, mHingeAxis2)
	ACPH_ADD_ATTRIBUTE(GearConstraintSettings, mRatio)
}
...
bool GearConstraint::SolvePositionConstraint(float inDeltaTime, float inBaumgarte)
{
	...
	float error = CenterAngleAroundZero(fmod(gear1rot + mRatio * gear2rot, 2.0f * ACPH_PI));
	if (error == 0.0f)
		return false;
