...
#include <AsterCore/Physics/Constraints/ContactConstraintManager.h>
#include <AsterCore/Physics/Constraints/CalculateSolverSteps.h>
...
//#define ACPH_MANIFOLD_CACHE_DEBUG
...
ACPH_INLINE void ContactConstraintManager::WorldContactPoint::TemplatedCalculateFrictionAndNonPenetrationConstraintProperties(float inDeltaTime, float inGravityDeltaTimeDotNormal, const Body &inBody1, const Body &inBody2, float inInvM1, float inInvM2, Mat44Arg inInvI1, Mat44Arg inInvI2, RVec3Arg inWorldSpacePosition1, RVec3Arg inWorldSpacePosition2, Vec3Arg inWorldSpaceNormal, Vec3Arg inWorldSpaceTangent1, Vec3Arg inWorldSpaceTangent2, const ContactSettings &inSettings, float inMinVelocityForRestitution)
{
	...
	{
		ACPH_ASSERT(false); // Static vs static makes no sense
...
void ContactConstraintManager::ManifoldCache::Init(uint inMaxBodyPairs, uint inMaxContactConstraints, uint inCachedManifoldsSize)
{
	...
	ACPH_ASSERT(max_body_pairs == inMaxBodyPairs, "Cannot support this many body pairs!");
	ACPH_ASSERT(inMaxContactConstraints <= cMaxContactConstraintsLimit); // Should have been enforced by caller
...
#ifdef ACPH_MANIFOLD_CACHE_DEBUG
	Trace("ManifoldMap:");
	mCachedManifolds.TraceStats();
	Trace("BodyPairMap:");
	mCachedBodyPairs.TraceStats();
#endif // ACPH_MANIFOLD_CACHE_DEBUG
}
...
void ContactConstraintManager::ManifoldCache::SaveState(StateRecorder &inStream, const StateRecorderFilter *inFilter) const
{
	...
	for (const BPKeyValue *bp_kv : selected_bp)
	{
		...
		for (const MKeyValue *m_kv : all_m)
		{
			...
			ACPH_ASSERT((cm.mFlags & (uint16)CachedManifold::EFlags::CCDContact) == 0);
...
{
	ACPH_ASSERT(mConstraints == nullptr);
}
...
void ContactConstraintManager::Init(uint inMaxBodyPairs, uint inMaxContactConstraints)
{
	...
	ACPH_ASSERT(mMaxConstraints == inMaxContactConstraints, "Cannot support this many contact constraints!");
...
void ContactConstraintManager::GetContactsFromCache(ContactAllocator &ioContactAllocator, Body &inBody1, Body &inBody2, bool &outPairHandled, bool &outConstraintCreated)
{
	...
	do
	{
		ACPH_PROFILE("Add Constraint From Cached Manifold");
		...
		ACPH_ASSERT(input_cm.mNumContactPoints > 0); // There should be contact points in this manifold!
		...
		ACPH_ASSERT(settings.mIsSensor || !(body1->IsSensor() || body2->IsSensor()), "Sensors cannot be converted into regular bodies by a contact callback!");
				...
				|| (body2->IsDynamic() && settings.mInvMassScale2 != 0.0f)))
		{
			...
			ACPH_DET_LOG("GetContactsFromCache: id1: " << constraint.mBody1->GetID() << " id2: " << constraint.mBody2->GetID() << " key: " << constraint.mSortKey);
...
bool ContactConstraintManager::TemplatedAddContactConstraint(ContactAllocator &ioContactAllocator, BodyPairHandle inBodyPairHandle, Body &inBody1, Body &inBody2, const ContactManifold &inManifold)
{
	...
	ACPH_ASSERT(num_contact_points <= MaxContactPoints);
	ACPH_ASSERT(num_contact_points == (int)inManifold.mRelativeContactPointsOn2.size());
	...
	ACPH_ASSERT(settings.mIsSensor || !(inBody1.IsSensor() || inBody2.IsSensor()), "Sensors cannot be converted into regular bodies by a contact callback!");
			...
			|| (inBody2.IsDynamic() && settings.mInvMassScale2 != 0.0f)))
	{
		...
		ACPH_DET_LOG("TemplatedAddContactConstraint: id1: " << constraint.mBody1->GetID() << " id2: " << constraint.mBody2->GetID() << " key: " << constraint.mSortKey);
...
bool ContactConstraintManager::AddContactConstraint(ContactAllocator &ioContactAllocator, BodyPairHandle inBodyPairHandle, Body &inBody1, Body &inBody2, const ContactManifold &inManifold)
{
	...
	ACPH_DET_LOG("AddContactConstraint: id1: " << inBody1.GetID() << " id2: " << inBody2.GetID()
	...
	ACPH_ASSERT(inManifold.mWorldSpaceNormal.IsNormalized());
...
void ContactConstraintManager::OnCCDContactAdded(ContactAllocator &ioContactAllocator, const Body &inBody1, const Body &inBody2, const ContactManifold &inManifold, ContactSettings &outSettings)
{
	...
	ACPH_ASSERT(outSettings.mIsSensor || !(inBody1.IsSensor() || inBody2.IsSensor()), "Sensors cannot be converted into regular bodies by a contact callback!");
}
...
void ContactConstraintManager::SortContacts(uint32 *inConstraintIdxBegin, uint32 *inConstraintIdxEnd) const
{
	...
	QuickSort(inConstraintIdxBegin, inConstraintIdxEnd, [this](uint32 inLHS, uint32 inRHS) {
		...
		ACPH_ASSERT(inLHS == inRHS, "Hash collision, ordering will be inconsistent");
		return false;
	});
}
...
void ContactConstraintManager::FinalizeContactCacheAndCallContactPointRemovedCallbacks(uint inExpectedNumBodyPairs, uint inExpectedNumManifolds)
{
	...
	ACPH_ASSERT(old_write_cache.GetNumBodyPairs() == inExpectedNumBodyPairs);
	ACPH_ASSERT(old_write_cache.GetNumManifolds() == inExpectedNumManifolds);
#endif
...
template <EMotionType Type1, EMotionType Type2>
ACPH_INLINE void ContactConstraintManager::sWarmStartConstraint(ContactConstraint &ioConstraint, MotionProperties *ioMotionProperties1, MotionProperties *ioMotionProperties2, float inWarmStartImpulseRatio)
{
	// Calculate tangents
	Vec3 t1, t2;
	ioConstraint.GetTangents(t1, t2);
...
void ContactConstraintManager::WarmStartVelocityConstraints(const uint32 *inConstraintIdxBegin, const uint32 *inConstraintIdxEnd, float inWarmStartImpulseRatio, MotionPropertiesCallback &ioCallback)
{
	...
	for (const uint32 *constraint_idx = inConstraintIdxBegin; constraint_idx < inConstraintIdxEnd; ++constraint_idx)
	{
		...
		{
			ACPH_ASSERT(motion_type2 == EMotionType::Dynamic);
...
template <EMotionType Type1, EMotionType Type2>
ACPH_INLINE bool ContactConstraintManager::sSolveVelocityConstraint(ContactConstraint &ioConstraint, MotionProperties *ioMotionProperties1, MotionProperties *ioMotionProperties2)
{
