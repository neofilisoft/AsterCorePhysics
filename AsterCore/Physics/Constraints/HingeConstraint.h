...
class ACPH_EXPORT HingeConstraintSettings final : public TwoBodyConstraintSettings
{
	ACPH_DECLARE_SERIALIZABLE_VIRTUAL(ACPH_EXPORT, HingeConstraintSettings)
...
public:
	...
	float						mLimitsMin = -ACPH_PI;
	float						mLimitsMax = ACPH_PI;
...
class ACPH_EXPORT HingeConstraint final : public TwoBodyConstraint
{
public:
	...
	// Motor controls
	void						SetMotorState(EMotorState inState)						{ ACPH_ASSERT(inState == EMotorState::Off || mMotorSettings.IsValid()); mMotorState = inState; }
