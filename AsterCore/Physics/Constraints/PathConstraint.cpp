...
#include <AsterCore/Physics/Constraints/PathConstraint.h>
...
ACPH_IMPLEMENT_SERIALIZABLE_VIRTUAL(PathConstraintSettings)
{
	ACPH_ADD_BASE_CLASS(PathConstraintSettings, TwoBodyConstraintSettings)

	ACPH_ADD_ATTRIBUTE(PathConstraintSettings, mPath)
	ACPH_ADD_ATTRIBUTE(PathConstraintSettings, mPathPosition)
	ACPH_ADD_ATTRIBUTE(PathConstraintSettings, mPathRotation)
	ACPH_ADD_ATTRIBUTE(PathConstraintSettings, mPathFraction)
	ACPH_ADD_ATTRIBUTE(PathConstraintSettings, mMaxFrictionForce)
	ACPH_ADD_ATTRIBUTE(PathConstraintSettings, mPositionMotorSettings)
	ACPH_ADD_ENUM_ATTRIBUTE(PathConstraintSettings, mRotationConstraintType)
}
...
bool PathConstraint::SolveVelocityConstraint(float inDeltaTime)
{
	...
	if (mPositionLimitsConstraintPart.IsActive())
	{
		...
		{
			ACPH_ASSERT(mPathFraction >= mPath->GetPathMaxFraction());
...
{
	ACPH_ASSERT(false); // Not implemented yet
	return nullptr;
}
