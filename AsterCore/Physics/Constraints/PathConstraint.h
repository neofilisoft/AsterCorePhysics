...
#include <AsterCore/Physics/Constraints/ConstraintPart/HingeRotationConstraintPart.h>
#include <AsterCore/Physics/Constraints/ConstraintPart/RotationEulerConstraintPart.h>

ACPH_NAMESPACE_BEGIN

ACPH_SUPPRESS_WARNING_PUSH
ACPH_GCC_SUPPRESS_WARNING("-Wshadow") // GCC complains about the 'Free' value conflicting with the 'Free' method
...
class ACPH_EXPORT PathConstraintSettings final : public TwoBodyConstraintSettings
{
	ACPH_DECLARE_SERIALIZABLE_VIRTUAL(ACPH_EXPORT, PathConstraintSettings)
...
class ACPH_EXPORT PathConstraint final : public TwoBodyConstraint
{
public:
	...
	void							SetPositionMotorState(EMotorState inState)				{ ACPH_ASSERT(inState == EMotorState::Off || mPositionMotorSettings.IsValid()); mPositionMotorState = inState; }
	...
	void							SetTargetPathFraction(float inFraction)					{ ACPH_ASSERT(mPath->IsLooping() || (inFraction >= 0.0f && inFraction <= mPath->GetPathMaxFraction())); mTargetPathFraction = inFraction; }
