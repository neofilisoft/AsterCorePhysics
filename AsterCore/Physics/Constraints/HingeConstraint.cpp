...
#include <AsterCore/Physics/Constraints/ConstraintPart/RotationEulerConstraintPart.h>
...
ACPH_IMPLEMENT_SERIALIZABLE_VIRTUAL(HingeConstraintSettings)
{
	ACPH_ADD_BASE_CLASS(HingeConstraintSettings, TwoBodyConstraintSettings)

	ACPH_ADD_ENUM_ATTRIBUTE(HingeConstraintSettings, mSpace)
	ACPH_ADD_ATTRIBUTE(HingeConstraintSettings, mPoint1)
	ACPH_ADD_ATTRIBUTE(HingeConstraintSettings, mHingeAxis1)
	ACPH_ADD_ATTRIBUTE(HingeConstraintSettings, mNormalAxis1)
	ACPH_ADD_ATTRIBUTE(HingeConstraintSettings, mPoint2)
	ACPH_ADD_ATTRIBUTE(HingeConstraintSettings, mHingeAxis2)
	ACPH_ADD_ATTRIBUTE(HingeConstraintSettings, mNormalAxis2)
	ACPH_ADD_ATTRIBUTE(HingeConstraintSettings, mLimitsMin)
	ACPH_ADD_ATTRIBUTE(HingeConstraintSettings, mLimitsMax)
	ACPH_ADD_ATTRIBUTE(HingeConstraintSettings, mLimitsSpringSettings)
	ACPH_ADD_ATTRIBUTE(HingeConstraintSettings, mMaxFrictionTorque)
	ACPH_ADD_ATTRIBUTE(HingeConstraintSettings, mMotorSettings)
}
...
{
	// Store limits
	ACPH_ASSERT(inSettings.mLimitsMin != inSettings.mLimitsMax || inSettings.mLimitsSpringSettings.mFrequency > 0.0f, "Better use a fixed constraint in this case");
...
{
	ACPH_ASSERT(inLimitsMin <= 0.0f && inLimitsMin >= -ACPH_PI);
	ACPH_ASSERT(inLimitsMax >= 0.0f && inLimitsMax <= ACPH_PI);
	mLimitsMin = inLimitsMin;
	mLimitsMax = inLimitsMax;
	mHasLimits = mLimitsMin > -ACPH_PI || mLimitsMax < ACPH_PI;
}
