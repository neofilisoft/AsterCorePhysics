...
#include <AsterCore/Physics/Constraints/SixDOFConstraint.h>
#include <AsterCore/Physics/Body/Body.h>
#include <AsterCore/Geometry/Ellipse.h>
...
ACPH_IMPLEMENT_SERIALIZABLE_VIRTUAL(SixDOFConstraintSettings)
{
	ACPH_ADD_BASE_CLASS(SixDOFConstraintSettings, TwoBodyConstraintSettings)

	ACPH_ADD_ENUM_ATTRIBUTE(SixDOFConstraintSettings, mSpace)
	ACPH_ADD_ATTRIBUTE(SixDOFConstraintSettings, mPosition1)
	ACPH_ADD_ATTRIBUTE(SixDOFConstraintSettings, mAxisX1)
	ACPH_ADD_ATTRIBUTE(SixDOFConstraintSettings, mAxisY1)
	ACPH_ADD_ATTRIBUTE(SixDOFConstraintSettings, mPosition2)
	ACPH_ADD_ATTRIBUTE(SixDOFConstraintSettings, mAxisX2)
	ACPH_ADD_ATTRIBUTE(SixDOFConstraintSettings, mAxisY2)
	ACPH_ADD_ATTRIBUTE(SixDOFConstraintSettings, mMaxFriction)
	ACPH_ADD_ENUM_ATTRIBUTE(SixDOFConstraintSettings, mSwingType)
	ACPH_ADD_ATTRIBUTE(SixDOFConstraintSettings, mLimitMin)
	ACPH_ADD_ATTRIBUTE(SixDOFConstraintSettings, mLimitMax)
	ACPH_ADD_ATTRIBUTE(SixDOFConstraintSettings, mLimitsSpringSettings)
	ACPH_ADD_ATTRIBUTE(SixDOFConstraintSettings, mMotorSettings)
}
...
void SixDOFConstraint::UpdateRotationLimits()
{
	...
	for (int i = EAxis::RotationX; i <= EAxis::RotationZ; ++i)
	{
		...
		mLimitMin[i] = Clamp(mLimitMin[i], -ACPH_PI, ACPH_PI);
		mLimitMax[i] = Clamp(mLimitMax[i], -ACPH_PI, ACPH_PI);
...
void SixDOFConstraint::UpdateFixedFreeAxis()
{
	...
	{
		float limit = a >= EAxis::RotationX? ACPH_PI : FLT_MAX;
...
{
	ACPH_ASSERT(inState == EMotorState::Off || mMotorSettings[inAxis].IsValid());
	...
	if (mMotorState[inAxis] != inState)
	{
		...
		{
			ACPH_ASSERT(inAxis >= EAxis::RotationX && inAxis <= EAxis::RotationZ);
	...
	if (IsTranslationFullyConstrained())
		...
		for (int i = 0; i < 3; ++i)
			if (mTranslationConstraintPart[i].IsActive())
			{
				...
				{
					ACPH_ASSERT(!IsFreeAxis(EAxis(EAxis::TranslationX + i)));
	...
	else if (IsTranslationConstrained())
	{
		...
		for (int i = 0; i < 3; ++i)
			if (mLimitsSpringSettings[i].mFrequency <= 0.0f) // If not soft limit
			{
				...
				default:	ACPH_ASSERT(i == 2); translation_axis = constraint_body1_to_world.RotateAxisZ(); break;
				}
