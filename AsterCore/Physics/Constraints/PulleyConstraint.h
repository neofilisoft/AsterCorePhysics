...
#include <AsterCore/Physics/Constraints/ConstraintPart/IndependentAxisConstraintPart.h>
...
class ACPH_EXPORT PulleyConstraintSettings final : public TwoBodyConstraintSettings
{
	ACPH_DECLARE_SERIALIZABLE_VIRTUAL(ACPH_EXPORT, PulleyConstraintSettings)
...
/// A pulley constraint.
class ACPH_EXPORT PulleyConstraint final : public TwoBodyConstraint
{
public:
	...
	void						SetLength(float inMinLength, float inMaxLength)				{ ACPH_ASSERT(inMinLength >= 0.0f && inMinLength <= inMaxLength); mMinLength = inMinLength; mMaxLength = inMaxLength; }
