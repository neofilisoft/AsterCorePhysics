...
public:
	/// Constructor
	ACPH_INLINE explicit			CalculateSolverSteps(const PhysicsSettings &inSettings) : mSettings(inSettings) { }

	/// Combine the number of velocity and position steps for this body/constraint with the current values
	template <class Type>
	ACPH_INLINE void				operator () (const Type *inObject)
	{
	...
	ACPH_INLINE uint				GetNumPositionSteps() const					{ return mNumPositionSteps; }
	ACPH_INLINE uint				GetNumVelocitySteps() const					{ return mNumVelocitySteps; }
...
public:
	template <class Type>
	ACPH_INLINE void				operator () (const Type *) const
	{
		/* Nothing to do */
	}
};
