...
void DestructionSystem::SaveSnapshotState(StateRecorder &inStream, const BodyInterface &inBodyInterface) const
{
	...
	for (const BodyID &body_id : body_ids)
	{
		...
		inStream.Write(state->mHasPendingDamage);
		inStream.Write(state->mPendingFracture);
		inStream.Write(state->mIsFractured);
		inStream.Write(state->mCollapseEventSent);
		...
		inStream.Write(intact_body_added);
		...
		inStream.Write(has_intact_template);
...
bool DestructionSystem::RestoreSnapshotState(StateRecorder &inStream, BodyInterface &inBodyInterface)
{
	...
	for (uint32 index = 0; index < num_breakable_bodies; ++index)
	{
		...
		inStream.Read(state->mHasPendingDamage);
		inStream.Read(state->mPendingFracture);
		inStream.Read(state->mIsFractured);
		inStream.Read(state->mCollapseEventSent);
		inStream.Read(state->mIntactBodyAdded);
		...
		inStream.Read(state->mHasIntactBodyTemplate);
...
bool DestructionSystem::FractureBody(BodyInterface &inBodyInterface, const BodyID &inBodyID, BreakableBodyState &ioState, EActivation inActivationMode)
{
	...
	for (uint32 chunk_index = 0; chunk_index < chunk_count; ++chunk_index)
	{
		...
		Body *body = inBodyInterface.CreateBody(settings);
		if (body == nullptr)
		{
...
{
	for (const BodyID &body_id : inBodyIDs)
		DestroyBodyIfPresent(inBodyInterface, body_id);
}
