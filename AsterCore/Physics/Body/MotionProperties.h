...
#include <AsterCore/Physics/Body/AllowedDOFs.h>
#include <AsterCore/Physics/Body/MotionQuality.h>
#include <AsterCore/Physics/Body/BodyAccess.h>
#include <AsterCore/Physics/Body/MotionType.h>
#include <AsterCore/Physics/Body/BodyType.h>
#include <AsterCore/Physics/Body/MassProperties.h>
#include <AsterCore/Physics/DeterminismLog.h>
...
class ACPH_EXPORT MotionProperties
{
public:
	...
	inline Vec3				GetLinearVelocity() const										{ ACPH_ASSERT(BodyAccess::sCheckRights(BodyAccess::sVelocityAccess(), BodyAccess::EAccess::Read)); return mLinearVelocity; }

	/// Set world space linear velocity of the center of mass
	void					SetLinearVelocity(Vec3Arg inLinearVelocity)						{ ACPH_ASSERT(BodyAccess::sCheckRights(BodyAccess::sVelocityAccess(), BodyAccess::EAccess::ReadWrite)); ACPH_ASSERT(inLinearVelocity.Length() <= mMaxLinearVelocity); mLinearVelocity = LockTranslation(inLinearVelocity); }
	...
	inline Vec3				GetAngularVelocity() const										{ ACPH_ASSERT(BodyAccess::sCheckRights(BodyAccess::sVelocityAccess(), BodyAccess::EAccess::Read)); return mAngularVelocity; }

	/// Set world space angular velocity of the center of mass
	void					SetAngularVelocity(Vec3Arg inAngularVelocity)					{ ACPH_ASSERT(BodyAccess::sCheckRights(BodyAccess::sVelocityAccess(), BodyAccess::EAccess::ReadWrite)); ACPH_ASSERT(inAngularVelocity.Length() <= mMaxAngularVelocity); mAngularVelocity = LockAngular(inAngularVelocity); }
	...
	inline void				SetMaxLinearVelocity(float inLinearVelocity)					{ ACPH_ASSERT(inLinearVelocity >= 0.0f); mMaxLinearVelocity = inLinearVelocity; }
	...
	inline void				SetMaxAngularVelocity(float inAngularVelocity)					{ ACPH_ASSERT(inAngularVelocity >= 0.0f); mMaxAngularVelocity = inAngularVelocity; }
	///@}
	...
	void					SetLinearDamping(float inLinearDamping)							{ ACPH_ASSERT(inLinearDamping >= 0.0f); mLinearDamping = inLinearDamping; }
	...
	void					SetAngularDamping(float inAngularDamping)						{ ACPH_ASSERT(inAngularDamping >= 0.0f); mAngularDamping = inAngularDamping; }
	...
	inline float			GetInverseMass() const											{ ACPH_ASSERT(mCachedMotionType == EMotionType::Dynamic); return mInvMass; }
	...
	inline Vec3				GetInverseInertiaDiagonal() const								{ ACPH_ASSERT(mCachedMotionType == EMotionType::Dynamic); return mInvInertiaDiagonal; }
	...
	ACPH_INLINE Vec3			MultiplyWorldSpaceInverseInertiaByVector(QuatArg inBodyRotation, Vec3Arg inV) const;

	/// Velocity of point inPoint (in center of mass space, e.g. on the surface of the body) of the body (unit: m/s)
	ACPH_INLINE Vec3			GetPointVelocityCOM(Vec3Arg inPointRelativeToCOM) const			{ return mLinearVelocity + mAngularVelocity.Cross(inPointRelativeToCOM); }
	...
	ACPH_INLINE Vec3			GetAccumulatedForce() const										{ return Vec3::sLoadFloat3Unsafe(mForce); }
	...
	ACPH_INLINE Vec3			GetAccumulatedTorque() const									{ return Vec3::sLoadFloat3Unsafe(mTorque); }

	// Reset the total accumulated force, note that this will be done automatically after every time step.
	ACPH_INLINE void			ResetForce()													{ mForce = Float3(0, 0, 0); }

	// Reset the total accumulated torque, note that this will be done automatically after every time step.
	ACPH_INLINE void			ResetTorque()													{ mTorque = Float3(0, 0, 0); }

	// Reset the current velocity and accumulated force and torque.
	ACPH_INLINE void			ResetMotion()
	{
		ACPH_ASSERT(BodyAccess::sCheckRights(BodyAccess::sVelocityAccess(), BodyAccess::EAccess::ReadWrite));
	...
	ACPH_INLINE UVec4		GetLinearDOFsMask() const
	{
	...
	ACPH_INLINE Vec3			LockTranslation(Vec3Arg inV) const
	{
	...
	ACPH_INLINE UVec4		GetAngularDOFsMask() const
	{
	...
	ACPH_INLINE Vec3			LockAngular(Vec3Arg inV) const
	{
	...
	void					SetNumVelocityStepsOverride(uint inN)							{ ACPH_ASSERT(inN < 256); mNumVelocityStepsOverride = uint8(inN); }
	...
	void					SetNumPositionStepsOverride(uint inN)							{ ACPH_ASSERT(inN < 256); mNumPositionStepsOverride = uint8(inN); }
	...
	///@{
	inline void				AddLinearVelocityStep(Vec3Arg inLinearVelocityChange)			{ ACPH_DET_LOG("AddLinearVelocityStep: " << inLinearVelocityChange); ACPH_ASSERT(BodyAccess::sCheckRights(BodyAccess::sVelocityAccess(), BodyAccess::EAccess::ReadWrite)); mLinearVelocity = LockTranslation(mLinearVelocity + inLinearVelocityChange); ACPH_ASSERT(!mLinearVelocity.IsNaN()); }
	inline void				SubLinearVelocityStep(Vec3Arg inLinearVelocityChange)			{ ACPH_DET_LOG("SubLinearVelocityStep: " << inLinearVelocityChange); ACPH_ASSERT(BodyAccess::sCheckRights(BodyAccess::sVelocityAccess(), BodyAccess::EAccess::ReadWrite)); mLinearVelocity = LockTranslation(mLinearVelocity - inLinearVelocityChange); ACPH_ASSERT(!mLinearVelocity.IsNaN()); }
	inline void				AddAngularVelocityStep(Vec3Arg inAngularVelocityChange)			{ ACPH_DET_LOG("AddAngularVelocityStep: " << inAngularVelocityChange); ACPH_ASSERT(BodyAccess::sCheckRights(BodyAccess::sVelocityAccess(), BodyAccess::EAccess::ReadWrite)); mAngularVelocity += inAngularVelocityChange; ACPH_ASSERT(!mAngularVelocity.IsNaN()); }
	inline void				SubAngularVelocityStep(Vec3Arg inAngularVelocityChange)			{ ACPH_DET_LOG("SubAngularVelocityStep: " << inAngularVelocityChange); ACPH_ASSERT(BodyAccess::sCheckRights(BodyAccess::sVelocityAccess(), BodyAccess::EAccess::ReadWrite)); mAngularVelocity -= inAngularVelocityChange; ACPH_ASSERT(!mAngularVelocity.IsNaN()); }
	///@}
