...
#include <AsterCore/Core/QuickSort.h>
	...
	#include <AsterCore/Physics/Body/BodyFilter.h>
...
inline void BodyManager::sDeleteBody(Body *inBody)
{
	...
	{
		ACPH_IF_ENABLE_ASSERTS(inBody->mMotionProperties = nullptr;)
...
{
	UniqueLock lock(mBodiesMutex ACPH_IF_ENABLE_ASSERTS(, this, EPhysicsLockTypes::BodiesList));
...
void BodyManager::Init(uint inMaxBodies, uint inNumBodyMutexes, const BroadPhaseLayerInterface &inLayerInterface)
{
	...
	for (BodyID *&active_bodies : mActiveBodies)
	{
		ACPH_ASSERT(active_bodies == nullptr);
...
Body *BodyManager::AllocateBody(const BodyCreationSettings &inBodyCreationSettings) const
{
	...
	if (inBodyCreationSettings.HasMassProperties())
	{
		...
		ACPH_IF_ENABLE_ASSERTS(mp->mCachedBodyType = body->mBodyType;)
		ACPH_IF_ENABLE_ASSERTS(mp->mCachedMotionType = body->mMotionType;)
	}
...
{
	ACPH_ASSERT(inBody->GetID().IsInvalid(), "This function should only be called on a body that doesn't have an ID yet, use DestroyBody otherwise");
...
bool BodyManager::AddBody(Body *ioBody)
{
	...
	uint32 idx;
	{
		...
		if (mBodyIDFreeListStart != cBodyIDFreeListEnd)
		{
			...
			ACPH_ASSERT(mBodyIDFreeListStart & cIsFreedBody);
			idx = uint32(mBodyIDFreeListStart >> cFreedBodyIndexShift);
			ACPH_ASSERT(!sIsValidBodyPointer(mBodies[idx]));
...

	{
		...
		if (idx < mBodies.size())
		{
			...
			else
			{
				...
				ACPH_ASSERT(cur != cBodyIDFreeListEnd >> cFreedBodyIndexShift);
			}
...
Body *BodyManager::RemoveBodyInternal(const BodyID &inBodyID)
{
	...
	ACPH_ASSERT(body->GetID() == inBodyID);
	ACPH_ASSERT(!body->IsActive());
	ACPH_ASSERT(!body->IsInBroadPhase(), "Use BodyInterface::RemoveBody to remove this body first!");
...
void BodyManager::ValidateFreeList() const
{
	...
	{
		ACPH_ASSERT(start & cIsFreedBody);
		num_freed++;
	}
	ACPH_ASSERT(mNumBodies == mBodies.size() - num_freed);
}

#endif // defined(ACPH_DEBUG) && _defined(ACPH_ENABLE_ASSERTS)
...
void BodyManager::RemoveBodies(const BodyID *inBodyIDs, int inNumber, Body **outBodies)
{
	...
	// Update cached number of bodies
	ACPH_ASSERT(mNumBodies >= (uint)inNumber);
	mNumBodies -= inNumber;
...
void BodyManager::AddBodyToActiveBodies(Body &ioBody)
{
	...
	ACPH_ASSERT(num_active_bodies_val < GetMaxBodies());
...
void BodyManager::RemoveBodyFromActiveBodies(Body &ioBody)
{
	...
	if (mp->mIndexInActiveBodies != last_body_index)
	{
		...
		ACPH_ASSERT(last_body.mMotionProperties->mIndexInActiveBodies == last_body_index);
...
void BodyManager::ActivateBodies(const BodyID *inBodyIDs, int inNumber)
{
	...
	UniqueLock lock(mActiveBodiesMutex ACPH_IF_ENABLE_ASSERTS(, this, EPhysicsLockTypes::ActiveBodiesList));

	ACPH_ASSERT(!mActiveBodiesLocked || sOverrideAllowActivation);
	...
	for (const BodyID *b = inBodyIDs, *b_end = inBodyIDs + inNumber; b < b_end; b++)
		if (!b->IsInvalid())
		{
			...
			ACPH_ASSERT(body.GetID() == body_id);
			ACPH_ASSERT(body.IsInBroadPhase(), "Use BodyInterface::AddBody to add the body first!");
...
void BodyManager::DeactivateBodies(const BodyID *inBodyIDs, int inNumber)
{
	...
	ACPH_ASSERT(!mActiveBodiesLocked || sOverrideAllowDeactivation);
...
void BodyManager::SetMotionQuality(Body &ioBody, EMotionQuality inMotionQuality)
{
	...
	if (mp != nullptr && mp->GetMotionQuality() != inMotionQuality)
	{
		...
		ACPH_ASSERT(!mActiveBodiesLocked);
...
void BodyManager::GetBodyIDs(BodyIDVector &outBodies) const
{
	...
	ACPH_ASSERT(outBodies.size() == mNumBodies);
}
...
{
	ACPH_ASSERT(sizeof(MutexMask) * 8 >= mBodyMutexes.GetNumMutexes(), "MutexMask must have enough bits");
...
{
	ACPH_IF_ENABLE_ASSERTS(PhysicsLock::sCheckLock(this, EPhysicsLockTypes::PerBody));
...
{
	ACPH_IF_ENABLE_ASSERTS(PhysicsLock::sCheckUnlock(this, EPhysicsLockTypes::PerBody));
...
void BodyManager::LockAllBodies() const
{
	...
	PhysicsLock::sLock(mBodiesMutex ACPH_IF_ENABLE_ASSERTS(, this, EPhysicsLockTypes::BodiesList));
}

void BodyManager::UnlockAllBodies() const
{
	PhysicsLock::sUnlock(mBodiesMutex ACPH_IF_ENABLE_ASSERTS(, this, EPhysicsLockTypes::BodiesList));
...
bool BodyManager::RestoreState(StateRecorder &inStream)
{
...

	{
		...
		if (inStream.IsValidating())
		{
			...
			{
				ACPH_ASSERT(false, "Cannot handle adding/removing bodies");
				UnlockAllBodies();
				return false;
			}
		...
		else
		{
			...
			for (uint32 idx = 0; idx < num_bodies; ++idx)
			{
				...
				if (b == nullptr)
				{
					ACPH_ASSERT(false, "Restoring state for non-existing body");
					UnlockAllBodies();
					return false;
				}
				bool is_active;
...
void BodyManager::Draw(const DrawSettings &inDrawSettings, const PhysicsSettings &inPhysicsSettings, DebugRenderer *inRenderer, const BodyDrawFilter *inBodyFilter)
{
	...
	for (const Body *body : mBodies)
		...
		{
			ACPH_ASSERT(mBodies[body->GetID().GetIndex()] == body);
			...
			if (inDrawSettings.mDrawSleepStats && body->IsDynamic() && body->IsActive())
			{
				...
				for (int i = 0; i < 3; ++i)
					inRenderer->DrawWireSphere(ACPH_IF_DOUBLE_PRECISION(body->mMotionProperties->GetSleepTestOffset() +) body->mMotionProperties->mSleepTestSpheres[i].GetCenter(), body->mMotionProperties->mSleepTestSpheres[i].GetRadius(), sleep_color);
			}
