...
public:
	...
	explicit				BodyID(uint32 inID) :
		mID(inID)
	{
		ACPH_ASSERT((inID & cBroadPhaseBit) == 0 || inID == cInvalidBodyID); // Check bit used by broadphase
	}
	...
	{
		ACPH_ASSERT(inID <= cMaxBodyIndex); // Should not overlap with broadphase bit or sequence number
	}
...
// Create a std::hash/ACPH::Hash for BodyID
ACPH_MAKE_HASHABLE(ACPH::BodyID, t.GetIndexAndSequenceNumber())
