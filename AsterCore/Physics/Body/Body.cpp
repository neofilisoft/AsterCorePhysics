...
void Body::SetMotionType(EMotionType inMotionType)
{
	...
	ACPH_ASSERT(inMotionType == EMotionType::Static || mMotionProperties != nullptr, "Body needs to be created with mAllowDynamicOrKinematic set to true");
	ACPH_ASSERT(inMotionType != EMotionType::Static || !IsActive(), "Deactivate body first");
	ACPH_ASSERT(inMotionType == EMotionType::Dynamic || !IsSoftBody(), "Soft bodies can only be dynamic, you can make individual vertices kinematic by setting their inverse mass to 0");
	...
	{
		// Update cache
		ACPH_IF_ENABLE_ASSERTS(mMotionProperties->mCachedMotionType = inMotionType;)
...
{
	ACPH_ASSERT(IsRigidBody()); // Only valid for rigid bodies
	ACPH_ASSERT(!IsStatic());
	ACPH_ASSERT(BodyAccess::sCheckRights(BodyAccess::sPositionAccess(), BodyAccess::EAccess::Read));
...
{
	ACPH_ASSERT(BodyAccess::sCheckRights(BodyAccess::sPositionAccess(), BodyAccess::EAccess::ReadWrite));
...
void Body::GetSubmergedVolume(RVec3Arg inSurfacePosition, Vec3Arg inSurfaceNormal, float &outTotalVolume, float &outSubmergedVolume, Vec3 &outRelativeCenterOfBuoyancy) const
{
	...
	mShape->GetSubmergedVolume(rotation, Vec3::sOne(), surface_relative_to_body, outTotalVolume, outSubmergedVolume, outRelativeCenterOfBuoyancy ACPH_IF_DEBUG_RENDERER(, mPosition));
}
...
{
	ACPH_ASSERT(IsRigidBody()); // Only implemented for rigid bodies currently
...
void Body::RestoreState(StateRecorder &inStream)
{
	...
	if (mMotionProperties != nullptr)
	{
		...
		ACPH_IF_ENABLE_ASSERTS(mMotionProperties->mCachedMotionType = mMotionType);
	}
