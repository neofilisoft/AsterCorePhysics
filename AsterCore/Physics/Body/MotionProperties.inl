...
void MotionProperties::MoveKinematic(Vec3Arg inDeltaPosition, QuatArg inDeltaRotation, float inDeltaTime)
{
	...
	ACPH_ASSERT(mCachedBodyType == EBodyType::RigidBody);
	ACPH_ASSERT(mCachedMotionType != EMotionType::Static);
...
{
	ACPH_ASSERT(mInvMass > 0.0f, "Body must have finite mass");
	ACPH_ASSERT(inMass > 0.0f, "New mass cannot be zero");
...
{
	ACPH_ASSERT(mCachedMotionType == EMotionType::Dynamic);
