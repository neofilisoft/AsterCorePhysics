...
#include <AsterCore/Core/MutexArray.h>
...
class ACPH_EXPORT BodyManager : public NonCopyable
{
public:
	...
	public:
		...
		{
			ACPH_ASSERT(!sGetOverrideAllowActivation());
			sSetOverrideAllowActivation(inAllowActivation);

			ACPH_ASSERT(!sGetOverrideAllowDeactivation());
...
private:
	...
	ACPH_INLINE Body *				RemoveBodyInternal(const BodyID &inBodyID);
