...
#include <AsterCore/Math/Matrix.h>
#include <AsterCore/Math/Vector.h>
#include <AsterCore/Math/EigenValueSymmetric.h>
#include <AsterCore/ObjectStream/TypeDeclarations.h>
#include <AsterCore/Core/StreamIn.h>
#include <AsterCore/Core/StreamOut.h>
#include <AsterCore/Core/InsertionSort.h>

ACPH_NAMESPACE_BEGIN

ACPH_IMPLEMENT_SERIALIZABLE_NON_VIRTUAL(MassProperties)
{
	ACPH_ADD_ATTRIBUTE(MassProperties, mMass)
	ACPH_ADD_ATTRIBUTE(MassProperties, mInertia)
}
...
bool MassProperties::DecomposePrincipalMomentsOfInertia(Mat44 &outRotation, Vec3 &outDiagonal) const
{
	...
	for (int i = 0; i < 3; ++i)
		ACPH_ASSERT(new_inertia.GetColumn3(i).IsClose(mInertia.GetColumn3(i), mInertia.GetColumn3(i).LengthSq() * 1.0e-10f));
#endif
