...
#include <AsterCore/Physics/Collision/Shape/Shape.h>
...
#include <AsterCore/Physics/Body/MotionProperties.h>
#include <AsterCore/Physics/Body/BodyID.h>
#include <AsterCore/Physics/Body/BodyAccess.h>
#include <AsterCore/Physics/Body/BodyType.h>
...
class
#ifndef ACPH_PLATFORM_DOXYGEN // Doxygen gets confused here
	ACPH_EXPORT_GCC_BUG_WORKAROUND alignas(max(ACPH_VECTOR_ALIGNMENT, ACPH_RVECTOR_ALIGNMENT))
#endif
...
public:
	...
	inline void				SetIsSensor(bool inIsSensor)									{ ACPH_ASSERT(IsRigidBody()); if (inIsSensor) mFlags.fetch_or(uint8(EFlags::IsSensor), memory_order_relaxed); else mFlags.fetch_and(uint8(~uint8(EFlags::IsSensor)), memory_order_relaxed); }
	...
	inline void				SetCollideKinematicVsNonDynamic(bool inCollide)					{ ACPH_ASSERT(IsRigidBody()); if (inCollide) mFlags.fetch_or(uint8(EFlags::CollideKinematicVsNonDynamic), memory_order_relaxed); else mFlags.fetch_and(uint8(~uint8(EFlags::CollideKinematicVsNonDynamic)), memory_order_relaxed); }
	...
	inline void				SetUseManifoldReduction(bool inUseReduction)					{ ACPH_ASSERT(IsRigidBody()); if (inUseReduction) mFlags.fetch_or(uint8(EFlags::UseManifoldReduction), memory_order_relaxed); else mFlags.fetch_and(uint8(~uint8(EFlags::UseManifoldReduction)), memory_order_relaxed); }
	...
	inline void				SetApplyGyroscopicForce(bool inApply)							{ ACPH_ASSERT(IsRigidBody()); if (inApply) mFlags.fetch_or(uint8(EFlags::ApplyGyroscopicForce), memory_order_relaxed); else mFlags.fetch_and(uint8(~uint8(EFlags::ApplyGyroscopicForce)), memory_order_relaxed); }
	...
	inline void				SetEnhancedInternalEdgeRemoval(bool inApply)					{ ACPH_ASSERT(IsRigidBody()); if (inApply) mFlags.fetch_or(uint8(EFlags::EnhancedInternalEdgeRemoval), memory_order_relaxed); else mFlags.fetch_and(uint8(~uint8(EFlags::EnhancedInternalEdgeRemoval)), memory_order_relaxed); }
	...
	void					SetLinearVelocity(Vec3Arg inLinearVelocity)						{ ACPH_ASSERT(!IsStatic()); mMotionProperties->SetLinearVelocity(inLinearVelocity); }
	...
	/// If you want the body to wake up when it is sleeping, use BodyInterface::SetLinearVelocity instead.
	void					SetLinearVelocityClamped(Vec3Arg inLinearVelocity)				{ ACPH_ASSERT(!IsStatic()); mMotionProperties->SetLinearVelocityClamped(inLinearVelocity); }
	...
	void					SetAngularVelocity(Vec3Arg inAngularVelocity)					{ ACPH_ASSERT(!IsStatic()); mMotionProperties->SetAngularVelocity(inAngularVelocity); }
	...
	/// If you want the body to wake up when it is sleeping, use BodyInterface::SetAngularVelocity instead.
	void					SetAngularVelocityClamped(Vec3Arg inAngularVelocity)			{ ACPH_ASSERT(!IsStatic()); mMotionProperties->SetAngularVelocityClamped(inAngularVelocity); }
	...
	inline Vec3				GetPointVelocity(RVec3Arg inPoint) const						{ ACPH_ASSERT(BodyAccess::sCheckRights(BodyAccess::sPositionAccess(), BodyAccess::EAccess::Read)); return GetPointVelocityCOM(Vec3(inPoint - mPosition)); }
	...
	inline void				AddForce(Vec3Arg inForce)										{ ACPH_ASSERT(IsDynamic()); (Vec3::sLoadFloat3Unsafe(mMotionProperties->mForce) + inForce).StoreFloat3(&mMotionProperties->mForce); }
	...
	inline void				AddTorque(Vec3Arg inTorque)										{ ACPH_ASSERT(IsDynamic()); (Vec3::sLoadFloat3Unsafe(mMotionProperties->mTorque) + inTorque).StoreFloat3(&mMotionProperties->mTorque); }
	...
	inline Vec3				GetAccumulatedForce() const										{ ACPH_ASSERT(IsDynamic()); return mMotionProperties->GetAccumulatedForce(); }
	...
	inline Vec3				GetAccumulatedTorque() const									{ ACPH_ASSERT(IsDynamic()); return mMotionProperties->GetAccumulatedTorque(); }

	// Reset the total accumulated force, not that this will be done automatically after every time step.
	ACPH_INLINE void			ResetForce()													{ ACPH_ASSERT(IsDynamic()); return mMotionProperties->ResetForce(); }

	// Reset the total accumulated torque, not that this will be done automatically after every time step.
	ACPH_INLINE void			ResetTorque()													{ ACPH_ASSERT(IsDynamic()); return mMotionProperties->ResetTorque(); }

	// Reset the current velocity and accumulated force and torque.
	ACPH_INLINE void			ResetMotion()													{ ACPH_ASSERT(!IsStatic()); return mMotionProperties->ResetMotion(); }
	...
	inline RVec3			GetPosition() const												{ ACPH_ASSERT(BodyAccess::sCheckRights(BodyAccess::sPositionAccess(), BodyAccess::EAccess::Read)); return mPosition - mRotation * mShape->GetCenterOfMass(); }

	/// World space rotation of the body
	inline Quat				GetRotation() const												{ ACPH_ASSERT(BodyAccess::sCheckRights(BodyAccess::sPositionAccess(), BodyAccess::EAccess::Read)); return mRotation; }
	...
	inline RVec3			GetCenterOfMassPosition() const									{ ACPH_ASSERT(BodyAccess::sCheckRights(BodyAccess::sPositionAccess(), BodyAccess::EAccess::Read)); return mPosition; }
	...
	inline void				ValidateCachedBounds() const
	{
		...
		ACPH_ASSERT(actual_body_bounds == mBounds, "Mismatch between cached bounding box and actual bounding box");
	}
#endif // ACPH_ENABLE_ASSERTS

	/// Access to the motion properties
	const MotionProperties *GetMotionProperties() const										{ ACPH_ASSERT(!IsStatic()); return mMotionProperties; }
	MotionProperties *		GetMotionProperties()											{ ACPH_ASSERT(!IsStatic()); return mMotionProperties; }
	...
	inline TransformedShape	GetTransformedShape() const										{ ACPH_ASSERT(BodyAccess::sCheckRights(BodyAccess::sPositionAccess(), BodyAccess::EAccess::Read)); return TransformedShape(mPosition, mRotation, mShape, mID); }
	...
	inline void				AddPositionStep(Vec3Arg inLinearVelocityTimesDeltaTime)			{ ACPH_ASSERT(IsRigidBody()); ACPH_ASSERT(BodyAccess::sCheckRights(BodyAccess::sPositionAccess(), BodyAccess::EAccess::ReadWrite)); mPosition += mMotionProperties->LockTranslation(inLinearVelocityTimesDeltaTime); ACPH_ASSERT(!mPosition.IsNaN()); }
	inline void				SubPositionStep(Vec3Arg inLinearVelocityTimesDeltaTime)			{ ACPH_ASSERT(IsRigidBody()); ACPH_ASSERT(BodyAccess::sCheckRights(BodyAccess::sPositionAccess(), BodyAccess::EAccess::ReadWrite)); mPosition -= mMotionProperties->LockTranslation(inLinearVelocityTimesDeltaTime); ACPH_ASSERT(!mPosition.IsNaN()); }
	...
	inline void				ValidateContactCacheInternal()									{ ACPH_IF_ENABLE_ASSERTS(uint8 old_val = ) mFlags.fetch_and(uint8(~uint8(EFlags::InvalidateContactCache)), memory_order_relaxed); ACPH_ASSERT((old_val & uint8(EFlags::InvalidateContactCache)) != 0); }
...
private:
	...
	explicit				Body(bool);														///< Alternative constructor that initializes all members
							...
							~Body()															{ ACPH_ASSERT(mMotionProperties == nullptr); } ///< Bodies must be destroyed through BodyInterface::DestroyBody
...
static_assert(ACPH_CPU_ADDRESS_BITS != 64 || ACPH_RVECTOR_ALIGNMENT < 16 || sizeof(Body) == ACPH_IF_SINGLE_PRECISION_ELSE(128, 160), "Body size is incorrect");
static_assert(alignof(Body) == max(ACPH_VECTOR_ALIGNMENT, ACPH_RVECTOR_ALIGNMENT), "Body should properly align");
