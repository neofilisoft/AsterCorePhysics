...
{
	ACPH_ASSERT(mMappings.empty() && mChains.empty() && mUnmapped.empty()); // Should not be initialized yet

	// Count joints
	int n1 = inSkeleton1->GetJointCount();
	int n2 = inSkeleton2->GetJointCount();
	ACPH_ASSERT(n1 <= n2, "Skeleton 1 should be the low detail skeleton!");
...
{
	ACPH_ASSERT(inSkeleton2->AreJointsCorrectlyOrdered());
...
{
	ACPH_ASSERT(!mMappings.empty(), "Call Initialize first!");
	ACPH_ASSERT(inSkeleton2->AreJointsCorrectlyOrdered());
	...
	int n = inSkeleton2->GetJointCount();
	bool *locked_translations = (bool *)ACPH_STACK_ALLOC(n * sizeof(bool));
...
void SkeletonMapper::Map(const Mat44 *inPose1ModelSpace, const Mat44 *inPose2LocalSpace, Mat44 *outPose2ModelSpace) const
{
	...
	for (const Unmapped &u : mUnmapped)
		...
		{
			ACPH_ASSERT(u.mParentJointIdx < u.mJointIdx, "Joints must be ordered: parents first");
