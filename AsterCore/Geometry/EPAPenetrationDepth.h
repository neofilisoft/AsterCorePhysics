...
#include <AsterCore/Geometry/EPAConvexHullBuilder.h>

//#define ACPH_EPA_PENETRATION_DEPTH_DEBUG
...
public:
	...
	{
		ACPH_IF_ENABLE_ASSERTS(mGJKTolerance = inTolerance;)
	...
	bool				GetPenetrationDepthStepEPA(const AI &inAIncludingConvexRadius, const BI &inBIncludingConvexRadius, float inTolerance, Vec3 &outV, Vec3 &outPointA, Vec3 &outPointB)
	{
		...
		ACPH_ASSERT(inTolerance >= FLT_EPSILON);
		...
		case 1:
			{
				...
				ACPH_ASSERT(support_points.mY[0].IsNearZero(Square(mGJKTolerance)));
...
#ifdef ACPH_EPA_PENETRATION_DEPTH_DEBUG
				Trace("Converged");
#endif // ACPH_EPA_PENETRATION_DEPTH_DEBUG
				break;
			}
	...
	{
		ACPH_IF_ENABLE_ASSERTS(mGJKTolerance = inCollisionTolerance;)
