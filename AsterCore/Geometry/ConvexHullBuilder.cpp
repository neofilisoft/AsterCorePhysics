...
#include <AsterCore/Geometry/ConvexHullBuilder.h>
#include <AsterCore/Geometry/ConvexHullBuilder2D.h>
#include <AsterCore/Geometry/ClosestPoint.h>
#include <AsterCore/Core/StringTools.h>
#include <AsterCore/Core/UnorderedSet.h>

#ifdef ACPH_CONVEX_BUILDER_DUMP_SHAPE
ACPH_SUPPRESS_WARNINGS_STD_BEGIN
#include <fstream>
ACPH_SUPPRESS_WARNINGS_STD_END
#endif // ACPH_CONVEX_BUILDER_DUMP_SHAPE

#ifdef ACPH_CONVEX_BUILDER_DEBUG
...
{
	ACPH_ASSERT(mFirstEdge == nullptr);
	ACPH_ASSERT(inIdx0 != inIdx1 && inIdx0 != inIdx2 && inIdx1 != inIdx2);
...
void ConvexHullBuilder::AddPoint(Face *inFacingFace, int inIdx, float inCoplanarToleranceSq, Faces &outNewFaces)
{
	...
	ACPH_ASSERT(edges.size() >= 3);

	// Create new faces
	outNewFaces.reserve(edges.size());
	for (const FullEdge &e : edges)
	{
		ACPH_ASSERT(e.mStartIdx != e.mEndIdx);
...
void ConvexHullBuilder::FreeFace(Face *inFace)
{
	...
	if (e != nullptr)
		do
		{
			ACPH_ASSERT(e->mNeighbourEdge == nullptr);
			e = e->mNextEdge;
...
void ConvexHullBuilder::sLinkFace(Edge *inEdge1, Edge *inEdge2)
{
	...
	ACPH_ASSERT(inEdge1->mNeighbourEdge == nullptr);
	ACPH_ASSERT(inEdge2->mNeighbourEdge == nullptr);
	ACPH_ASSERT(inEdge1->mFace != inEdge2->mFace);

	// Check vertices match
	ACPH_ASSERT(inEdge1->mStartIdx == inEdge2->mNextEdge->mStartIdx);
	ACPH_ASSERT(inEdge2->mStartIdx == inEdge1->mNextEdge->mStartIdx);
...
void ConvexHullBuilder::sUnlinkFace(Face *inFace)
{
	...
	do
	{
		if (e->mNeighbourEdge != nullptr)
		{
			...
			ACPH_ASSERT(e->mNeighbourEdge->mNeighbourEdge == e);
...
void ConvexHullBuilder::FindEdge(Face *inFacingFace, Vec3Arg inVertex, FullEdges &outEdges) const
{
	...
	ACPH_ASSERT(outEdges.empty());

	// Should start with a facing face
	ACPH_ASSERT(inFacingFace->IsFacing(inVertex));
			...
			if (ne != nullptr)
			{
				...
				if (!n->mRemoved)
				{
					...
					if (n->IsFacing(inVertex))
					{
						...
						cur_stack_pos++;
						ACPH_ASSERT(cur_stack_pos < cMaxEdgeLength);
	...
	for (int i = 0; i < (int)outEdges.size(); ++i)
		ACPH_ASSERT(outEdges[i].mEndIdx == outEdges[(i + 1) % outEdges.size()].mStartIdx);
#endif
...
bool ConvexHullBuilder::RemoveTwoEdgeFace(Face *inFace, Faces &ioAffectedFaces) const
{
	...
	Edge *edge = inFace->mFirstEdge;
	Edge *next_edge = edge->mNextEdge;
	ACPH_ASSERT(edge != next_edge); // 1 edge faces should not exist
...
#endif // ACPH_CONVEX_BUILDER_DEBUG
