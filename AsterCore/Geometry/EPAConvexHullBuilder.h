...
public:
...
#ifdef ACPH_EPA_CONVEX_BUILDER_VALIDATE
	...
	void				FreeTriangle(Triangle *inT)
	{
		...
		ACPH_ASSERT(inT->mRemoved);
		for (const Edge &e : inT->mEdge)
			ACPH_ASSERT(e.mNeighbourTriangle == nullptr);
#endif

#if defined(ACPH_EPA_CONVEX_BUILDER_VALIDATE) || defined(ACPH_EPA_CONVEX_BUILDER_DRAW)
		// Remove from list of all triangles
		Triangles::iterator i = std::find(mTriangles.begin(), mTriangles.end(), inT);
		ACPH_ASSERT(i != mTriangles.end());
		mTriangles.erase(i);
#endif
...
private:
	...
	{
		ACPH_ASSERT(inEdge1 >= 0 && inEdge1 < 3);
		ACPH_ASSERT(inEdge2 >= 0 && inEdge2 < 3);
		Edge &e1 = inT1->mEdge[inEdge1];
		Edge &e2 = inT2->mEdge[inEdge2];

		// Check not connected yet
		ACPH_ASSERT(e1.mNeighbourTriangle == nullptr);
		ACPH_ASSERT(e2.mNeighbourTriangle == nullptr);

		// Check vertices match
		ACPH_ASSERT(e1.mStartIdx == inT2->GetNextEdge(inEdge2).mStartIdx);
		ACPH_ASSERT(e2.mStartIdx == inT1->GetNextEdge(inEdge1).mStartIdx);
	...
	void				UnlinkTriangle(Triangle *inT)
	{
		...
		for (int i = 0; i < 3; ++i)
		{
			...
			if (edge.mNeighbourTriangle != nullptr)
			{
				...
				ACPH_ASSERT(neighbour_edge.mNeighbourTriangle == inT);
				ACPH_ASSERT(neighbour_edge.mNeighbourEdge == i);
	...
	bool				FindEdge(Triangle *inFacingTriangle, Vec3Arg inVertex, Edges &outEdges)
	{
		...
		ACPH_ASSERT(inFacingTriangle->IsFacing(inVertex));
		...
		ACPH_ASSERT(outEdges.empty() || outEdges[0].mStartIdx == next_expected_start_idx);
	...
	void				ValidateTriangle(const Triangle *inT) const
	{
		if (inT->mRemoved)
		{
			...
			for (const Edge &my_edge : inT->mEdge)
				ACPH_ASSERT(my_edge.mNeighbourTriangle == nullptr);
		}
		else
		{
			for (int i = 0; i < 3; ++i)
			{
				...
				ACPH_ASSERT(nb != nullptr);

				if (nb != nullptr)
				{
					// Assert that our neighbours edge points to us
					const Edge &nb_edge = nb->mEdge[my_edge.mNeighbourEdge];
					ACPH_ASSERT(nb_edge.mNeighbourTriangle == inT);
					ACPH_ASSERT(nb_edge.mNeighbourEdge == i);
