...
class ACPH_EXPORT ObjectStream : public NonCopyable
{
public:
	/// Stream type
...
class ACPH_EXPORT IObjectStreamIn : public ObjectStream
{
public:
...
class ACPH_EXPORT IObjectStreamOut : public ObjectStream
{
public:
...
#define ACPH_DECLARE_PRIMITIVE(name)																			\
	ACPH_EXPORT bool	OSIsType(name *, int inArrayDepth, EOSDataType inDataType, const char *inClassName);	\
	ACPH_EXPORT bool	OSReadData(IObjectStreamIn &ioStream, name &outPrimitive);								\
	ACPH_EXPORT void	OSWriteDataType(IObjectStreamOut &ioStream, name *);									\
	ACPH_EXPORT void	OSWriteData(IObjectStreamOut &ioStream, const name &inPrimitive);
...
{
	return ioStream.ReadPointerData(ACPH_RTTI(T), inRef.InternalGetPointer(), T::sInternalGetRefCountOffset());
}
