...
public:
	...
	inline const uint8 *		GetData() const														{ ACPH_ASSERT(IsLockedForRead()); return mData; }
	inline uint8 *				GetData()															{ ACPH_ASSERT(IsLockedForWrite()); return mData; }
	inline int					GetStride() const													{ ACPH_ASSERT(IsLocked()); return mStride; }
	inline int					GetLength() const													{ ACPH_ASSERT(IsLocked()); return mLength; }

	/// Get start of a specific scanline
	inline const uint8 *		GetScanLine(int inScanLine) const									{ ACPH_ASSERT(inScanLine >= 0 && inScanLine < GetHeight()); return GetData() + inScanLine * GetStride(); }
	inline uint8 *				GetScanLine(int inScanLine)											{ ACPH_ASSERT(inScanLine >= 0 && inScanLine < GetHeight()); return GetData() + inScanLine * GetStride(); }
