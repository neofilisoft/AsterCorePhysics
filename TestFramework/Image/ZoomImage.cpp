...
static const ImageFilter &GetFilter(EFilter inFilter)
{
	...
	switch (inFilter)
	{
		...
		default:				ACPH_ASSERT(false);	return mitchell;
	}
}
...
{
	ACPH_PROFILE("PrecalculateFilter");
	...
	for (int i = 0; i < inNewLength; ++i)
	{
		...
		for (int source = left; source <= right; ++source)
		{
			...
			ACPH_ASSERT(c.mOffset >= 0 && c.mOffset < inOldLength);
...
{
	ACPH_PROFILE("ZoomHorizontal");

	// Check zoom parameters
	ACPH_ASSERT(inSrc->GetHeight() == ioDst->GetHeight());
...
{
	ACPH_PROFILE("ZoomVertical");

	// Check zoom parameters
	ACPH_ASSERT(inSrc->GetWidth() == ioDst->GetWidth());
	ACPH_ASSERT(inSrc->GetFormat() == ioDst->GetFormat());
