...
{
	ACPH_PROFILE("sComputeTranslationTable");
...
{
	ACPH_PROFILE("sConvertImageDifferentTypes");
	...
	ACPH_ASSERT(width == ioDst->GetWidth());
	ACPH_ASSERT(height == ioDst->GetHeight());
...
{
	ACPH_PROFILE("sConvertImageSameTypes");

	// Get image properties
	int dbpp = ioDst->GetBytesPerPixel();
	int width = inSrc->GetWidth();
	int height = inSrc->GetHeight();
	ACPH_ASSERT(inSrc->GetFormat() == ioDst->GetFormat());
	ACPH_ASSERT(dbpp == inSrc->GetBytesPerPixel());
	ACPH_ASSERT(width == ioDst->GetWidth());
	ACPH_ASSERT(height == ioDst->GetHeight());
...
{
	ACPH_PROFILE("sConvertRGBToAlpha");

	// Check surface format
	ACPH_ASSERT(ioSurface->GetFormat() == ESurfaceFormat::A8R8G8B8);
...
{
	ACPH_PROFILE("sConvertAlphaToRGB");
...
{
	ACPH_PROFILE("sConvertToGrayScale");
