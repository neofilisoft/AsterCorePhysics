...
ACPH_CLANG_SUPPRESS_WARNING("-Wreserved-identifier")
ACPH_CLANG_SUPPRESS_WARNING("-Wzero-as-null-pointer-constant")
ACPH_CLANG_SUPPRESS_WARNING("-Wcast-qual")
ACPH_CLANG_SUPPRESS_WARNING("-Wimplicit-fallthrough")
ACPH_CLANG_SUPPRESS_WARNING("-Wcomma")
...
try_again:;
	...
	for (int c = cBeginChar + 1; c < cEndChar; ++c)
	{
		...
		ACPH_SCOPE_EXIT([bitmap]{ STBTT_free(bitmap, nullptr); });
		yoff = baseline + yoff;
		...
		ACPH_ASSERT(x >= 0 && x <= 0xffff);
		ACPH_ASSERT(y >= 0 && y <= 0xffff);
		ACPH_ASSERT(w <= 0xff);
...
bool Font::CreateString(Mat44Arg inTransform, const string_view &inText, ColorArg inColor, RenderPrimitive &ioPrimitive) const
{
	...
	ACPH_ASSERT(vtx == (uint32)vtx_size);
	ACPH_ASSERT(idx == idx_start + idx_size);
