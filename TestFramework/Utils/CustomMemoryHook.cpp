...
struct InCustomAllocator
{
	InCustomAllocator()
	{
		ACPH_ASSERT(!sInCustomAllocator);
...
{
	ACPH_ASSERT(inSize > 0 && inAlignment > 0 && inAlignment <= 64);
	InCustomAllocator ica;
...
{
	ACPH_ASSERT(!sEnableCustomMemoryHook || sDisableCustomMemoryHook <= 0 || sInCustomAllocator);
	return true;
}
...
#endif // ACPH_CUSTOM_MEMORY_HOOK_ENABLED
