...
TEST_SUITE("QuatTests")
{
	...
	TEST_CASE("TestQuatConvertMatrix")
	{
		...
		uniform_real_distribution<float> zero_to_two_pi(0.0f, 2.0f * ACPH_PI);
	...
	{
		Mat44 r1 = Mat44::sRotation(Vec3(1, 0, 0), 0.1f * ACPH_PI);
		Mat44 r2 = Mat44::sRotation(Quat::sRotation(Vec3(1, 0, 0), 0.1f * ACPH_PI));
		CHECK_APPROX_EQUAL(r1, r2);

		r1 = Mat44::sRotation(Vec3(0, 1, 0), 0.2f * ACPH_PI);
		r2 = Mat44::sRotation(Quat::sRotation(Vec3(0, 1, 0), 0.2f * ACPH_PI));
		CHECK_APPROX_EQUAL(r1, r2);

		r1 = Mat44::sRotation(Vec3(0, 0, 1), 0.3f * ACPH_PI);
		r2 = Mat44::sRotation(Quat::sRotation(Vec3(0, 0, 1), 0.3f * ACPH_PI));
		CHECK_APPROX_EQUAL(r1, r2);
	}
	...
	TEST_CASE("TestQuatGetAxisAngle")
	{
		...
		Quat q1 = Quat::sRotation(Vec3(0, 1, 0), 0.2f * ACPH_PI);

		{
			...
			CHECK_APPROX_EQUAL(0.2f * ACPH_PI, angle, 1.0e-5f);
		}
		...
		Quat q2 = Quat::sRotation(Vec3(0, 1, 0), -0.2f * ACPH_PI);
		...
		Quat q3 = Quat::sRotation(Vec3(0, 1, 0), 1.1f * ACPH_PI);

		{
			...
			CHECK_APPROX_EQUAL(Vec3(0, -1, 0), axis);
			CHECK_APPROX_EQUAL(0.9f * ACPH_PI, angle, 1.0e-5f);
		}
	...
	TEST_CASE("TestQuatSLERP")
	{
		...
		Quat v2 = Quat::sRotation(Vec3::sAxisX(), 0.99f * ACPH_PI);
		CHECK_APPROX_EQUAL(v1.SLERP(v2, 0.25f), Quat::sRotation(Vec3::sAxisX(), 0.25f * 0.99f * ACPH_PI));
