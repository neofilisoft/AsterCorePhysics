...
TEST_SUITE("Mat44Tests")
{
	...
	TEST_CASE("TestCenterAngleAroundZero")
	{
		...
		{
			CHECK_APPROX_EQUAL(CenterAngleAroundZero(i * ACPH_PI), 0, 1.0e-5f);
			CHECK_APPROX_EQUAL(CenterAngleAroundZero((0.5f + i) * ACPH_PI), 0.5f * ACPH_PI, 1.0e-5f);
			CHECK_APPROX_EQUAL(CenterAngleAroundZero((1.5f + i) * ACPH_PI), -0.5f * ACPH_PI, 1.0e-5f);
			CHECK_APPROX_EQUAL(CenterAngleAroundZero(-(0.5f + i) * ACPH_PI), -0.5f * ACPH_PI, 1.0e-5f);
			CHECK_APPROX_EQUAL(CenterAngleAroundZero(-(1.5f + i) * ACPH_PI), 0.5f * ACPH_PI, 1.0e-5f);
			CHECK_APPROX_EQUAL(CenterAngleAroundZero(-(0.99f + i) * ACPH_PI), -0.99f * ACPH_PI, 1.0e-5f);
			CHECK_APPROX_EQUAL(CenterAngleAroundZero((0.99f + i) * ACPH_PI), 0.99f * ACPH_PI, 1.0e-5f);
		}
	}
