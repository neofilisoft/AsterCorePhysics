...
TEST_SUITE("STLLocalAllocatorTest")
{
	...
	static void sTestArray()
	{
		...
		{
			CHECK(arr[i] == i);
		#if !defined(ACPH_USE_STD_VECTOR) && !defined(ACPH_DISABLE_CUSTOM_ALLOCATOR)
		...
		arr.shrink_to_fit();
		CHECK(arr.size() == 0);
	#ifndef ACPH_USE_STD_VECTOR // Some implementations of std::vector ignore shrink_to_fit
		...
		CHECK(IsAligned(arr.data(), alignof(typename ArrayType::value_type)));
	#if !defined(ACPH_USE_STD_VECTOR) && !defined(ACPH_DISABLE_CUSTOM_ALLOCATOR) // Doesn't work with std::vector since it doesn't use the reallocate function and runs out of space
		CHECK(sIsLocal(arr));
	#endif
