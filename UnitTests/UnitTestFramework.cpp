...
#include <AsterCore/ConfigurationString.h>
...
// Disable common warnings triggered by AsterCore
...
#ifdef ACPH_PLATFORM_WINDOWS_UWP

ACPH_SUPPRESS_WARNING_PUSH
ACPH_MSVC_SUPPRESS_WARNING(4265) // warning C4265: 'winrt::impl::implements_delegate<winrt::Windows::UI::Core::DispatchedHandler,H>': class has virtual functions, but its non-trivial destructor is not virtual; instances of this class may not be destructed correctly
ACPH_MSVC_SUPPRESS_WARNING(4668) // warning C4668: '_MANAGED' is not defined as a preprocessor macro, replacing with '0' for '#if/#elif'
ACPH_MSVC_SUPPRESS_WARNING(4946) // warning C4946: reinterpret_cast used between related classes: 'winrt::impl::abi<winrt::Windows::ApplicationModel::Core::IFrameworkViewSource,void>::type' and 'winrt::impl::abi<winrt::Windows::Foundation::IUnknown,void>::type'
...
ACPH_MSVC_SUPPRESS_WARNING(5204) // warning C5204: 'winrt::impl::produce_base<D,winrt::Windows::ApplicationModel::Core::IFrameworkViewSource,void>': class has virtual functions, but its trivial destructor is not virtual; instances of objects derived from this class may not be destructed correctly
ACPH_MSVC_SUPPRESS_WARNING(5246) // warning C5246: '_Elems': the initialization of a subobject should be wrapped in braces
...
struct App : implements<App, IFrameworkViewSource, IFrameworkView>
{
	...
	void SetWindow(CoreWindow const& inWindow)
	{
		...
		// Install callbacks
		Trace = TraceImpl;
		ACPH_IF_ENABLE_ASSERTS(AssertFailed = AssertFailedImpl;)
...
#elif !defined(ACPH_PLATFORM_ANDROID)
...
#if defined(ACPH_PLATFORM_WINDOWS) && defined(_DEBUG)
	// Enable leak detection
	_CrtSetDbgFlag(_CRTDBG_ALLOC_MEM_DF | _CRTDBG_LEAK_CHECK_DF);
#endif
...
#define REPORTER_OVERRIDE(func, type, arg)						\
	...
	{															\
		...
		if (!str.empty())										\
			__android_log_write(ANDROID_LOG_INFO, "AsterCore", str.c_str());	\
		mStream.str("");										\
	}
...
{
	// Log configuration
	__android_log_write(ANDROID_LOG_INFO, "AsterCore", GetConfigurationString());
	...
	ACPH::Color color = return_value == 0? ACPH::Color::sGreen : ACPH::Color::sRed;
