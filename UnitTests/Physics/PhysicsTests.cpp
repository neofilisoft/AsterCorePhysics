...
#include <AsterCore/Physics/Body/BodyLockMulti.h>
...
TEST_SUITE("PhysicsTests")
{
	...
	TEST_CASE("TestPhysicsPosition")
	{
		...
		Vec3 box_pos(1, 2, 3);
		Quat box_rotation = Quat::sRotation(Vec3::sAxisX(), 0.25f * ACPH_PI);

		// Translate / rotate the body
		RVec3 body_pos(4, 5, 6);
		Quat body_rotation = Quat::sRotation(Vec3::sAxisY(), 0.3f * ACPH_PI);
	...
	TEST_CASE("TestPhysicsOverrideMassAndInertia")
	{
		...
		const Quat cOverriddenInertiaRotation = Quat::sRotation(Vec3(1, 1, 1).Normalized(), 0.1f * ACPH_PI);
	...
	TEST_CASE("TestOutOfContactConstraints")
	{
		...
		// Step
		{
			ACPH_IF_ENABLE_ASSERTS(ExpectAssert expect_assert(1);)
