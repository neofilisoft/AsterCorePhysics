...
TEST_SUITE("TransformedShapeTests")
{
	TEST_CASE("TestTransformedShape")
	{
		...
		const Quat rtshape_rotation = Quat::sRotation(Vec3(1, 2, 3).Normalized(), 0.25f * ACPH_PI);
		const RVec3 translation(13, 9, 7);
		const Quat rotation = Quat::sRotation(Vec3::sAxisY(), 0.5f * ACPH_PI); // A rotation of 90 degrees in order to not shear the shape
