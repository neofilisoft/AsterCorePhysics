...
TEST_SUITE("SliderConstraintTests")
{
	...
	TEST_CASE("TestSliderReferenceFrame")
	{
		...
		PhysicsTestContext c;
		Body &body1 = c.CreateBox(RVec3(1, 2, 3), Quat::sRotation(Vec3(1, 1, 1).Normalized(), 0.1f * ACPH_PI), EMotionType::Dynamic, EMotionQuality::Discrete, Layers::MOVING, Vec3(1, 1, 1), EActivation::Activate);
		Body &body2 = c.CreateBox(RVec3(-3, -2, -1), Quat::sRotation(Vec3(1, 0, 1).Normalized(), 0.2f * ACPH_PI), EMotionType::Dynamic, EMotionQuality::Discrete, Layers::MOVING, Vec3(1, 1, 1), EActivation::Activate);
	...
	TEST_CASE("TestSliderSpring")
	{
		...
		for (int mode = 0; mode < 2; ++mode)
		{
			...
			float omega = 2.0f * ACPH_PI * cFrequency;
