...
#include <AsterCore/Physics/DeterminismLog.h>
	...
	#include <AsterCore/Renderer/DebugRendererRecorder.h>
...
static void TraceImpl(const char *inFMT, ...)
{
	...
	// Print to the TTY
#ifndef ACPH_PLATFORM_ANDROID
	cout << buffer << endl;
#else
	__android_log_write(ANDROID_LOG_INFO, "AsterCore", buffer);
#endif
}
...
int main(int argc, char** argv)
{
	...
	for (int argidx = 1; argidx < argc; ++argidx)
	{
		...
		else if (strcmp(arg, "-h") == 0)
		{
			...
			Trace("Usage:\n"
				  ...
				  "-r: Record debug renderer output for AsterCoreViewer\n"
	...
	// Register all AsterCore physics types
	RegisterTypes();
	...
	for (int r = 0; r < repeat; ++r)
	{
		...
		for (uint mq = 0; mq < 2; ++mq)
		{
			...
			for (uint num_threads : thread_permutations)
			{
				...
				{
					ACPH_PROFILE_NEXTFRAME();
					ACPH_DET_LOG("Iteration: " << iterations);
					...
					{
						ACPH_PROFILE_DUMP(tag + "_it" + ConvertToString(iterations));
					}
					...
					for (BodyID id : body_ids)
					{
						...
						if (!body.IsStatic())
							ACPH_DET_LOG(id << ": p: " << body.GetPosition() << " r: " << body.GetRotation() << " v: " << body.GetLinearVelocity() << " w: " << body.GetAngularVelocity());
					}
